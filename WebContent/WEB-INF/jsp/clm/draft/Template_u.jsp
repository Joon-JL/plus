...
<%-- 
/**
 * 파  일  명 : Template_u.jsp
 * 프로그램명 : 표준 Template 수정
 * 설      명 : 
 * 작  성  자 : 
 * 작  성  일 : 
 */
...
<!--
	...
	*/
	function pageAction(flag){
		...
		if(flag == "insert"){
		    frm.method.value = "modifyTemplate";
		    ...
		    if(frm.biz_clsfcn.value == "" && frm.depth_clsfcn.value == "" && frm.cnclsnpurps_bigclsfcn.value == ""){
		    	alert("<spring:message code='clm.page.field.template.bizClsfcn' />, <spring:message code='clm.page.field.template.depthClsfcn' />, <spring:message code='clm.page.field.template.cnclsnpurpsClsfcn' /> : <spring:message code='secfw.msg.ask.cpAlert30' />");
		    	return;
		    }
		  	
			//거래상대방 체크
		    if(frm.cntrt_oppnt_cds == undefined){
		    	alert("<spring:message code='clm.page.field.template.cntrtOppntCd' />, <spring:message code='clm.page.field.template.depthClsfcn' />, <spring:message code='clm.page.field.template.cnclsnpurpsClsfcn' /> : <spring:message code='secfw.msg.ask.cpAlert30' />");
		    	return;
		    }
	...
	function customerClick(){
		var frm = document.frm;
		var customer = frm.customer.value;
		
		viewHiddenProgress(true);

		PopUpWindowOpen('', 1163, 550, true);
		frm.target = "PopUpWindow";
		frm.action = "<c:url value='/clm/draft/customer.do' />";
		frm.method.value = "listCustomer";
		frm.srch_customer.value = customer;
		frm.customer_gb.value   = "M";
		frm.submit();
	}
	
	//거래상대방 X 클릭 시.
	function customerImgClick(i){
		var frm = document.frm;
		
		frm.cntrt_oppnt_cds[i].value = "";
		frm.cntrt_oppnt_nms[i].value = "";
		document.getElementById("customer_"+i).style.display = "none";
	}
	...
	function returnCustomer(obj){
		...
		var objIndex = eval(obj.index) + eval(oppntCnt);
		
		var returnVal = "";
		returnVal += "<span id='customer_"+objIndex+"'><td>"+obj.customer_nm1+"</td>";
    	returnVal += "&nbsp;";
    	returnVal += "<a href='javascript:customerImgClick("+objIndex+");' ><img SRC='<%=IMAGE%>/icon/cancel_new.gif'/></a>";
    	returnVal += "<input type='hidden' id='cntrt_oppnt_cds"+objIndex+"' name='cntrt_oppnt_cds' value='" + obj.customer_cd + "' />";
    	returnVal += "<input type='hidden' id='cntrt_oppnt_nms"+objIndex+"' name='cntrt_oppnt_nms' value='" + obj.customer_nm1 + "' />";
    	returnVal += "</span>&nbsp;&nbsp;";
...
<input type="hidden" name="customer_gb"   id="customner_gb"     value="" />
...
<div id="wrap">
	...
	<div id="container">	
		...
		<!-- title -->
		<div class="title">
			<h3><spring:message code="clm.page.title.template.modifyTitle" /></h3>
		</div>
		<!-- //title -->
		...
		<div id="content">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table-style01">
				...
				<tbody>
					...
					<tr>
						<th><spring:message code="clm.page.field.template.langGbn" /><span class="astro">*</span></th>
						<td colspan="3">
							<select name="lang_gbn" id="lang_gbn" required alt="<spring:message code="clm.page.field.template.langGbn" />">
							</select>
						</td>
					</tr>
					<tr>
						<th><spring:message code="clm.page.field.template.cntrtOppntCd" /><span class="astro">*</span></th>
						<td colspan="3">
							<input type="text" id="customer" name="customer" onKeyPress="if(event.keyCode==13) {customerClick();return false;}" />
							<img src="<%=IMAGE%>/icon/ico_detail.gif" onClick="customerClick();" style="cursor:hand"/>
							...
							%>
								<span id="customer_<%=i%>"><%=oppnt.get("cntrt_oppnt_nm")%>
									<a href="javascript:customerImgClick(<%=i%>);"><img SRC='<%=IMAGE%>/icon/cancel_new.gif'/></a>
									<input id="cntrt_oppnt_cds<%=i%>" name="cntrt_oppnt_cds" type="hidden" value="<%=oppnt.get("cntrt_oppnt_cd")%>"/>
									<input id="cntrt_oppnt_nms<%=i%>" name="cntrt_oppnt_nms" type="hidden" value="<%=oppnt.get("cntrt_oppnt_nm")%>"/>
								</span>	
							<%	   
							   }
							} %>							
					...
					<tr>
						<th><spring:message code="clm.page.field.template.title" /><span class="astro">*</span></th>
						<td colspan="3">
							<input type="text" name="title" id="title" required alt="<spring:message code="clm.page.field.template.title" />" maxLength="300" class="text_full" style="width:600px" value="<%=lom.get("title")%>" />
						</td>
					</tr>
					<tr>
						<th><spring:message code="clm.page.field.template.cont" /><span class="astro">*</span></th>
						<td  colspan="3">
