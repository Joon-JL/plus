		...
		var code1 = code2 = chklist = "";
		   ...
		   if(chklist == "N"){
		    ...
		    if(confirm("<spring:message code='secfw.msg.ask.update' />")){
				...
				$("[id^='review_yn']").each(function(index) {
					resultYn += $(this).val()+"/";
				});
				...
				frm.rev_id_list.value =resultYn;
		    	frm.method.value = "modifyRole";
				frm.action = "<c:url value='/clm/admin/role.do' />";
				frm.target = "_self";
				viewHiddenProgress(true);
				frm.submit();
	...
	*/	
	...
	function add() {
		...
		if($("[id^='sign_td']").length > 0){
			...
			$("#trRelationContract").append("<tr id='sign_tr"+co_no+"'><td class='tC' id='td_val1'>"+co_no+"</td><td><p id='sign_td"+co_no+"' ><input type='text' name='sign_nm"+co_no+"' id='sign_nm"+co_no+"' style='width:60%'"
					...
					+"<span id='assign_nm"+co_no+"'></span><input type='hidden' id='user_id"+co_no+"' name='user_id"+co_no+"' value='' /><input type='hidden' id='review_yn"+co_no+"' name='review_yn"+co_no+"' value='N' /></td>"
...
<div id="wrap">
	...
	<div id="container">
		...
		<div id="content_in">
			...
			<!--reviewer parameter  -->
			<input type="hidden" id="rev_id_list" name="rev_id_list" value="" />
			<!-- 결재선검색 -->
			...
			<table class="table-style01">
				...
				<tbody>
					<tr>
						...
						<td colspan="3">
					  		<input type="text" name="role_name" id="role_name" class="text" style="width:174px"  value="<c:out value='${lom.role_name}'/>"/>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td colspan="3">
					  		<input type="text" name="description" id="description" class="text_full" value="<c:out value='${lom.description}'/>" alt="<spring:message code='clm.page.field.admin.type.cd_nm' />" maxLength="100" />
						</td>
					</tr>
					<tr>
						<!-- 지법인 선택 -->
	            ...
	            <table cellspacing="0" cellpadding="0" border="0" class="table-style01" id="trRelationContract">
	                ...
	                <c:forEach var="item" items="${resultListUser}" varStatus="i">
	                 <tr id="sign_tr<c:out value="${i.count}"/>">
	                     <td class='tC' id="td_val<c:out value="${i.count}"/>"><c:out value="${i.count}"/></td>
	                     
	                     <td>
	                      
	                        <span id="assign_nm<c:out value="${i.count}"/>"><c:out value="${item.user_nm}"/></span>
	                        <input type='hidden' id='user_id<c:out value="${i.count}"/>' name='user_id<c:out value="${i.count}"/>' value='<c:out value="${item.user_id}"/>' />
	                        <input type='hidden' id='review_yn<c:out value="${i.count}"/>' name='review_yn<c:out value="${i.count}"/>' value='<c:out value="${item.review_yn}"/>' />
                    	 </td>
