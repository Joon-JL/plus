...
<script language="javascript">
	...
	$(document).ready(function(){
		getCodeSelectByUpCd("frm", "srch_reg_operdiv", "/common/clmsCom.do?method=getComboHTML&combo_gbn=OPERDIV&combo_type=T&combo_del_yn=N&combo_add_yn=Y&combo_selected=<%=command.getSrch_reg_operdiv()%>");   
	...
	*/ 
	function pageAction(flag){
		...
		if(flag == "search"){
			...
			viewHiddenProgress(true) ;
		    frm.method.value = "forwardInsertBoard";
			frm.submit();
		}
	}
	...
	<div id="container">
							...
							<td>
								<table class="search_form">
									...
									<tr>
										...
										<th><spring:message code="clm.page.field.board.srchRegOperdiv" /></th>
										<td>
											<select name="srch_reg_operdiv" id="srch_reg_operdiv">
											</select>
										</td>
									</tr>
									<tr>										
								    	...
								    	<td colspan="3">
											...
											<input id="srch_keyword_content" name="srch_keyword_content" style="width:250px" value="<c:out value='${command.srch_keyword_content}'/>" onKeypress="if(event.keyCode == 13){pageAction('search'); return false;}"/>
										</td>
									</tr>
								</table>
							</td>
		...
		<%} %>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="list_basic">
				<colgroup>
			  	...
			  	<thead>
					<tr>
						...
						<th><spring:message code="clm.page.field.board.sec" /></th>
					</tr>
				</thead>
			 	<tbody>
				...
				%>
			        <tr <%if(idx == resultList.size()-1){%>class="end"<%}%> onmouseout="this.className='<%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onmouseover="this.className='on <%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onclick="javascript:detailView(<%=lom.get("seqno")%>)"> 
			          <td><%=lom.get("rn")%></td>
			          <td title="<%=(String)lom.get("orgnz_nms")%>"><nobr><%=(String)lom.get("rd_trgt1_nm")%></nobr></td>
			          <td title="<%=(String)lom.get("rd_trgt2_nm")%>"><nobr><%=(String)lom.get("rd_trgt2_nm")%></nobr></td>			          
			          <td class="tL txtover" title="<%=(String)lom.get("title")%>">
			          		<nobr>
			          		<%if(localeCode.equals("ko")){ %>
			          		<%=(String)lom.get("title")%>
			          		<%} else if(localeCode.equals("fr")){ %>
			          		<%=(String)lom.get("title_fr")%>
			          		<%} else if(localeCode.equals("de")){ %>
			          		<%=(String)lom.get("title_de")%>
			          		<%} else { %>
