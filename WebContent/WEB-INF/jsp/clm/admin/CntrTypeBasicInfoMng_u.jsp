...
<script language="javascript">
	...
	*/	
	function goSave(){
		...
		frm.method.value = "modifyTypeBasicInfoMng";
		frm.action = "<c:url value='/clm/admin/cntrTypeBasicInfoMng.do' />";
		frm.target = "_self";
		frm.submit();
	}
...
<div id="wrap">
	...
	<div id="container">
		...
		<!-- title -->
		<div class="title">
		  <h3><spring:message code="clm.page.cntrTypeBasicInfoMng.modifyTitle" /></h3><!-- 계약유형별 속성관리 수정 -->
		</div>
		<!-- //title -->
		...
		<!-- button -->
	  	<div class="btn_wrap_t03">
			...
			<span class="btn_all_w"><span class="edit"></span><a href="javascript:goSave();"><spring:message code="clm.page.msg.common.save" /></a></span>
...
<%
			for(int j=0; j < detaillist.size(); j++){
				...
				if(StringUtil.bvl(lom.get("attr_seqno"), "").equals(StringUtil.bvl(subLom.get("attr_seqno"), ""))){
					...
					String mndtryYn = StringUtil.bvl(subLom.get("mndtry_yn"), "");
					String attrSeqno = StringUtil.bvl(subLom.get("attr_seqno"), "");
					if("".equals(mndtryYn)){
%>
						&nbsp;
<%						
...
%>
						<select name="mndtry_yn_arr" style="width:200px;">
							<option value="Y" <% if ("Y".equals(mndtryYn)) {%> selected <%} %>><spring:message code="clm.field.cntrTypeBasicInfoMng.necessariness" /></option><!-- 필수 -->
							<option value="N" <% if ("N".equals(mndtryYn)) {%> selected <%} %>><spring:message code="clm.field.cntrTypeBasicInfoMng.choice" /></option><!-- 선택 -->
						</select>
						<input type="hidden" name="attr_seqno_arr" value="<%=attrSeqno%>"/>
						<%--<%=attrSeqno%>&nbsp;<%=mndtryYn%>&nbsp;
						<%=mndtryYn.equals("Y")? "필수":"선택" %>
						--%>
<%						
					}				
				}
...
%>
				<tr>
					<td class="td_col_03 tal_lineL"><%=lom.get("info_itm")%></td>
					<td>
<%
			for(int j=0; j < detaillist.size(); j++){
				ListOrderedMap subLom = (ListOrderedMap)detaillist.get(j);
