...
<%--
/**
 * 파  일  명 : RegConsideration_inner_i.jsp
 * 프로그램명 : 체결 후 등록 계약 등록 수정
 * 설      명 : 
 * 작  성  자 : 김재원
 * 작  성  일 : 2012.01
 */
		  ...
		  <colgroup>
			...
			<tr>
				...
				<td colspan="2">
					...
					<a href="javascript:openNegoCustomer();"><img src="<%=IMAGE %>/icon/ico_search.gif" class="cp" alt="search" /></a>
					<%}%></td>
				<th><spring:message code="clm.page.field.customer.registerNo" /></th>
				<td><input type="text" name="cntrt_oppnt_rprsntman" id="cntrt_oppnt_rprsntman" required alt="<spring:message code="clm.page.field.customer.registerNo" htmlEscape="true" />" value="<c:out value='${lomRq.cntrt_oppnt_rprsntman}'/>" maxlength="60" class="text_full" style="width:80%">
				</td>
				<th><spring:message code="clm.page.field.customer.contractRequired" /></th>
				<td><select name="cntrt_oppnt_type" id="cntrt_oppnt_type_nm"></select>
				</td>				
			</tr>
			<tr>
			...
			<tr class="slide-target02 slide-area">
				...
				<c:choose>
					<c:when test="${command.plndbn_req_yn == 'Y'}">
						<span><spring:message code="clm.page.msg.manage.announce050" /><br><spring:message code="clm.page.msg.manage.announce113" /></span>
					</c:when>
					<c:otherwise>
		...
		<div class="tal_subBox" id="tr_down02" style="display:none;">		
			...
			<table cellspacing="0" cellpadding="0" border="0" class="table-style01">
				...
				<tr>
					...
					<td colspan="2">
						<select id="payment_gbn" class="all" style="width:150px" onChange="paymentGbnChange();">						
						</select>
					</td>	
				...
				<tr>
					...
					<td colspan="4"><textarea name="antcptnefct" id="antcptnefct" alt="<spring:message code="clm.page.msg.manage.hope" htmlEscape="true" />" cols="30" rows="7" onkeyup="f_chk_byte(this,300)" class="text_area_full" maxLength="800"><c:out value='${lomRq.antcptnefct}'/></textarea></td>
				</tr>
				<tr>
				...
				<%			}											%>
								...
								<input type="text" name="arr_attr_cont" <%=dpMndtry %> id="arr_attr_cont" value="<%=(String)lom.get("attr_cont") %>" maxlength="240" class="text_full" style="margin-top:4px;" /></td>
				<%		}else if(j%2 == 1){								%>
							<th><span><%=(String)lom.get("attr_nm") %></span></th>
								<td colspan="3">
								<input type="hidden" name="arr_attr_seqno" id="arr_attr_seqno" value="<%=(Integer)lom.get("attr_seqno") %>">
			...
			<table id="tab_contents_sub_conts2" cellspacing="0" cellpadding="0" border="0" class="table-style01" style="display:none">
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="bfhdcstn_mtnman_nm" value="<c:out value='${lomRq.bfhdcstn_mtnman_nm}'/>" style="width:120px" class="text_search" onKeyPress="if(event.keyCode==13) {searchEmployee2('MTN');return false;}" /><a href="javascript:searchEmployee2('MTN');"><img src="<%=IMAGE %>/icon/ico_search.gif" class="cp" alt="search" /></a>
					</td>
