...
/**
 * 파  일  명 : Bbs_i.jsp
 * 프로그램명 : Q&A / 자유게시판 / 자료실 / 법무시스템매뉴얼 - 등록
						...
						if(frm.insert_kbn.value==0){
							<c:choose>
								<c:when test="${command.bbs_knd=='C01201'|| command.bbs_knd=='C01217'}">
									frm.method.value = "insertSysQnA";
								</c:when>
								<c:otherwise>
									frm.method.value = "insertOpenBbs";
								</c:otherwise>
							</c:choose>
						} else {
							<c:choose>
								<c:when test="${command.bbs_knd=='C01201'|| command.bbs_knd=='C01217'}">
									frm.method.value = "insertReplySysQnA";
								</c:when>
								<c:otherwise>
									frm.method.value = "insertReplyPds";
								</c:otherwise>
							</c:choose>
						}
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table-style01">
				...
				<tbody>
		        	<tr>
			            ...
			            <td colspan="3">
			            	<span><input type="text" name="title" id="title" class="text_full" maxLength="256" value="<c:out value="${title}" escapeXml="false"/>" alt="<spring:message code='las.page.field.board.notice.title'/>" maxLength="64" required /></span>
			            </td>
		        	</tr>
		        	<tr>
