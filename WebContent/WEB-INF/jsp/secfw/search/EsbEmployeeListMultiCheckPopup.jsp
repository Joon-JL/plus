...
<%--
/**
 * Subject   : EsbEmployeeListMultiPopup.jsp
 * Create On : 2013. 10.
 * 
 */
--%>
<%
	...
	ArrayList listUser    = (ArrayList)request.getAttribute("resultList");
...
<!--
	...
	function setUserInfos(button){
		...
		if(listSize == 1) {
			...
			var check_yn = frm.elements['chkValues'];
			var allcheck;
			var count = 0;
			
			for(i=0 ; i<check_yn.length;i++){
				
				if(check_yn[i].checked){
					count = count+1;
				}
			...
			if(allcheck == "Y"){
				opener.setUserInfosMulit(userList);
...
<body class="pop" onload="setUserInfos('');">
<h1><spring:message code="secfw.page.field.search.empSearch"/><span class="close" onclick="javascript:self.close();" title="close"></span></h1>
	...
	<div class="pop_area">
 	...
 	if(srchCntnt.indexOf(";") > 0){ 
 	
 	if(listUser != null && listUser.size()>0) {
 		for(int idx=0; idx<listUser.size(); idx++){
 			
 			Vector vt = (Vector)listUser.get(idx);
	...
	%> 
			<tr>
				<td>
				<% if(ht.get("check").equals("Y")){ %>
				        <input type="checkbox" checked="checked" id="<%=idx %>" name="chkValues" value="<%=ht.get("epid") %>" class="checkbox">
				<% } else { %>
				        <input type="checkbox" id="<%=idx %>" name="chkValues" value="<%=ht.get("epid") %>" class="checkbox">
				<% } %>
				</td>
						
				<td align="center"><%=ht.get("userid") %></td>
				<td align="center"><%=ht.get("cn") %></td>
				<td align="center"><div style='width:230;overflow:hidden;text-overflow:ellipsis'><nobr><%=ht.get("department") + "/" + ht.get("title") + "/" + ht.get("o") %></nobr></div></td>
...
<div class="pop_footer">
	<div class="btn_wrap tR" >
		<span class="btn_all_w"><span class="check"></span><a href="javascript:setUserInfos('X');" class="bt_all_b" style="cursor:hand;"><spring:message code="las.page.field.lawconsulting.select"/></a></span>
