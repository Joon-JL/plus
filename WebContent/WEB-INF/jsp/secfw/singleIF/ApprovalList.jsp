...
<title><spring:message code="secfw.page.title.singleIF.ApprovalList" /></title>
...
<!--
	...
	*/
	function detailView(module_id, mis_id){
		...
		frm.action = "<c:url value='/secfw/approval.do' />";
		frm.method.value = "detailApproval";
...
<!-- //**************************** Form Setting ****************************-->
		...
		<!-- Title -->
		<h3><spring:message code="secfw.page.title.singleIF.ApprovalList" /></h3>
		<!-- Search -->
						...
						<!-- 상태 -->
			              ...
			              <td>
			                <select name="srch_status" id="srch_status" class="select">
			                  ...
			                  <option value="1" <c:if test="${command.srch_status eq '1'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.proceedAppr"/></option>
			                  <option value="E" <c:if test="${command.srch_status eq '2'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.completed"/></option>
			                  <option value=""  <c:if test="${command.srch_status eq '3'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.reject"/></option>
			                  <option value="T" <c:if test="${command.srch_status eq '4'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.cancelConfirm"/></option>
			                  <option value="0" <c:if test="${command.srch_status eq '5'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.decArb"/></option>
			                  <option value="1" <c:if test="${command.srch_status eq '6'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.complAfter"/></option>
			                  <option value="E" <c:if test="${command.srch_status eq 'E'}">selected</c:if>><spring:message code="secfw.page.field.singleIF.error"/></option>
			                </select>
			              </td>
			              <td>
			                <!-- 조회 -->
		...
		<div class="dscrollx" style="height:293px;">
			<table class="basic_list mz fix" style="width:800px;" >
	          ...
	          <tr>
	            ...
	            <th><spring:message code="secfw.page.field.singleIF.sender"/></th><!-- 발신자결재주소 -->
	            <th><spring:message code="secfw.page.field.singleIF.onStatus"/></th><!-- 상태값 -->
	...
	%>
	          <tr onMouseOut="this.className='';" onMouseOver="this.className='selected'">
	            <td><%=lom.get("rn") %></td>
	            <td class="overflow"><%=lom.get("title") %></td>
	            <td><%=lom.get("draft_nm") %></td>
