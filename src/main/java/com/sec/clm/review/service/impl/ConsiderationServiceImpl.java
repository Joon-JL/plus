...
public class ConsiderationServiceImpl extends CommonServiceImpl implements ConsiderationService {
    ...
    public int modifyRefCCAJAX(ConsultationVO vo) throws Exception {
        ...
        if ("Y".equals(vo.getClient_modify_div())) {

            if (!"".equals(vo.getMaster_cntrt_id())) {
                rtVal = commonDAO.delete("las.contractmanager.deleteContractAuthreq", vo); // 권한요청 관련자 초기화

                String[] trgtIds = vo.getArr_trgtman_id();
                int arrayLength = (trgtIds == null) ? 0 : trgtIds.length;
...
//			}
        ...
        int_cnsd_level = Integer.parseInt(cnsd_level);

        // If cnsd_level is even number then +1
        if (int_cnsd_level % 2 == 0) {
