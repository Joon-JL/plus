	...
	public void listStatisticsExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    public void statistics_List(HttpServletRequest request, HttpServletResponse response) throws Exception{
	    	try{
				...
				vo.setSort_yn("Y");	//신성우 추가 통계 정렬순서위해서 2014-03-21`
				// 검색기간 초기값 세팅
				form.setSrch_start_dt(StringUtil.nvl(form.getSrch_start_dt(), "").equals("") ? DateUtil.formatDate(DateUtil.year() + "0101", "-") : form.getSrch_start_dt()) ;
				form.setSrch_end_dt(StringUtil.nvl(form.getSrch_end_dt(), "").equals("") ? DateUtil.formatDate(DateUtil.today(), "-") : form.getSrch_end_dt()) ;
				...
				//통계 데이터 가져오기
				ModelAndView listStatisticsMav = new ModelAndView();
				listStatisticsMav = listStatisticsContLegalAdvice(vo); 
				
				//Json object 형태로 변경처리
				JSONObject listStatisticsObj = new JSONObject();

				int startnum = 0;
				if (vo.getSrch_type() != null && vo.getSrch_type().equals("PL")) {
					startnum = 0;
				}else{
					startnum = 1;
				}
				...
				for (int i = startnum; i < listOrderMap.size(); i++) {
					...
					//label 텍스트 변경처리
					if(form.getSrch_type().toString().equals("PC")){
						//신성우 주석처리 draft saved 제외
						/*if(listOrderMap.get(i).toString().equals("draftsaved")){
							labelname = "Draft Saved(Review)";
						}else*/
						if(listOrderMap.get(i).toString().equals("review1")){
							labelname = "Review in Progress(Review)";
						}else if(listOrderMap.get(i).toString().equals("review2")){
							labelname = "Holding(Review)";
						}else if(listOrderMap.get(i).toString().equals("review3")){
							labelname = "Admin Replied(Review)";
						}else if(listOrderMap.get(i).toString().equals("review4")){
							labelname = "Replied(Review)";
						}else if(listOrderMap.get(i).toString().equals("conclusion1")){
							labelname = "Approval in Progress(Conclusion)";
						}else if(listOrderMap.get(i).toString().equals("conclusion2")){
							labelname = "Draft Saved(Conclusion)";
						}else if(listOrderMap.get(i).toString().equals("conclusion3")){
							labelname = "Holding(Conclusion)";
						}else if(listOrderMap.get(i).toString().equals("conclusion4")){
							labelname = "Cancel Approval(Conclusion)";
						}else if(listOrderMap.get(i).toString().equals("registration1")){
							labelname = "Registration Signing(Registration)";
						}else if(listOrderMap.get(i).toString().equals("registration2")){
							labelname = "Hard copy not Delivered(Registration)";
						}else if(listOrderMap.get(i).toString().equals("execution1")){
							labelname = "Execution In Progress";
						}else if(listOrderMap.get(i).toString().equals("termination1")){
							labelname = "Cancelled(Termination)";
						}else if(listOrderMap.get(i).toString().equals("termination2")){
							labelname = "Expired(Termination)";
						}else if(listOrderMap.get(i).toString().equals("termination3")){
							labelname = "Will be terminated(Termination)";
						}else if(listOrderMap.get(i).toString().equals("termination4")){
							labelname = "Approval in Progress(Termination)";
						}else if(listOrderMap.get(i).toString().equals("termination5")){
							labelname = "Extension Review(Termination)";
						}else if(listOrderMap.get(i).toString().equals("adminreplied")){
							labelname = "Admin Replied(Termination)";
						}else if(listOrderMap.get(i).toString().equals("ExecutedContract")){
							labelname = "Executed Contract";
						}
						colsObjectList.put("label",labelname);
					}else if (form.getSrch_type().toString().equals("PL")){
						if(listOrderMap.get(i).toString().equals("col1")){
							labelname = "Not Assigned";
						}else if(listOrderMap.get(i).toString().equals("col2")){
							labelname = "Review in Progress";
						}else if(listOrderMap.get(i).toString().equals("col3")){
							labelname = "Replied";
						}else if(listOrderMap.get(i).toString().equals("col4")){
							labelname = "Holding";
						}else if(listOrderMap.get(i).toString().equals("col5")){
							labelname = "Returned";
						}else if(listOrderMap.get(i).toString().equals("col6")){
							labelname = "Saved";
						}
						colsObjectList.put("label",labelname);
					}else{
						colsObjectList.put("label",listOrderMap.get(i));
					}
					//total 제외처리
					if(!listOrderMap.get(i).toString().equals("total") && !listOrderMap.get(i).toString().equals("draftsaved")){
						//parameter type 분기처리, 첫번째 필드를 제외한 나머지는 전부 숫자형
						if (i <= startnum) {
							colsObjectList.put("type","string");	
						}else { 
							colsObjectList.put("type","number");
						}
						
						cols += colsObjectList.toString();
				...
				//top 10 외 othertotal 합산내역
				if(ChartType.equals("Top10")){
					//listordermap의 사이즈만큼 others 배열 설정
					...
					for (int i = 0; i < listStatisticsArray.size(); i++) {
						...
						//list 10번째까지는 해당 형태로 처리..
						if (i < 10) {
							for (int j = startnum; j < listStatisticsMap.size(); j++) {
								//total, draftsaved 제외처리
								if(!listOrderMap.get(j).toString().equals("total") && !listOrderMap.get(j).toString().equals("draftsaved")){
									//법인코드 시작 위치 0,1 구분
									if (j <= startnum) {
										rowsObjectValue.put("v", listStatisticsMap.get(listOrderMap.get(j)));
									}else { 
										rowsObjectValue.put("v", Math.floor(Double.parseDouble(listStatisticsMap.get(listStatisticsMap.get(j)).toString().replace("-", "0"))));
									}
									rows += rowsObjectValue.toString();
							...
							for (int j = 0; j < listStatisticsMap.size(); j++) {
								...
								if(!listOrderMap.get(j).toString().equals("total") && !listOrderMap.get(j).toString().equals("draftsaved")){
									...
									if (j <= startnum) {
										...
										otherstotal[j] += Math.floor(Double.parseDouble(listStatisticsMap.get(listStatisticsMap.get(j)).toString().replace("-", "0")));
									}
								}
							}
						}
