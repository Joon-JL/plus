...
public class StatisticsController extends CommonController {
    ...
    private ModelAndView listStatisticsAssign(StatisticsVO vo) throws Exception {
        ...
        if (vo.getSrch_type().equals("C")) {
            if (vo.getSrch_div().equals("T01"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsAssign01", null, locale1);    //비지니스단계
            else if (vo.getSrch_div().equals("T02"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsAssign02", null, locale1);//계약단계
            else if (vo.getSrch_div().equals("T03"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsAssign03", null, locale1);//체결목적
        } else {
    ...
    private ModelAndView listStatisticsDept(StatisticsVO vo) throws Exception {
        ...
        if (vo.getSrch_type().equals("C")) {
            if (vo.getSrch_div().equals("T01"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsDept01", null, locale1);//비지니스단계
            else if (vo.getSrch_div().equals("T02"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsDept02", null, locale1);//계약단계
            else if (vo.getSrch_div().equals("T03"))
                divTitle = (String) messageSource.getMessage("las.page.field.statistics.listStatisticsDept03", null, locale1);//체결목적
        } else {
...
//                listStatisticsObj.put("rows", listStatisticsObj);
    ...
    private String getLabelName(String key, String srchType) {
        if ("PC".equals(srchType)) {
            switch (key) {
                case "review1":
                    return "Review in Progress(Review)";
                case "review2":
                    return "Holding(Review)";
                case "review3":
                    return "Admin Replied(Review)";
                case "review4":
                    return "Replied(Review)";
                case "conclusion1":
                    return "Approval in Progress(Conclusion)";
                case "conclusion2":
                    return "Draft Saved(Conclusion)";
                case "conclusion3":
                    return "Holding(Conclusion)";
                case "conclusion4":
                    return "Cancel Approval(Conclusion)";
                case "registration1":
                    return "Registration Signing(Registration)";
                case "registration2":
                    return "Hard copy not Delivered(Registration)";
                case "execution1":
                    return "Execution In Progress";
                case "termination1":
                    return "Cancelled(Termination)";
                case "termination2":
                    return "Expired(Termination)";
                case "termination3":
                    return "Will be terminated(Termination)";
                case "termination4":
                    return "Approval in Progress(Termination)";
                case "termination5":
                    return "Extension Review(Termination)";
                case "adminreplied":
                    return "Admin Replied(Termination)";
                case "ExecutedContract":
...
//        String ChartType = getChartType(form);
    ...
    private void exportToCsv(HttpServletResponse response, List dataList, String queryCode) throws Exception {
        ...
        if ("sesk".equals(queryCode)) {
            headers = new String[]{"Registration Year", "Registration Month", "Registration Date", "NERP Subsidiary", "SELMS+ Subsidiary", "Contract ID", "Request Title", "Contract Name", "Requesting dept.", "ESBO dept.", "Requester (In Charge)", "Request Date", "Area of contract", "Contract Type", "Contract Matter", "Contract start date", "Contract end date", "Contract amount", "Currency Unit", "Counterparty", "Conclusion date", "Step", "Status", "Reviewer", "GERP_CUSTOMER_NM", "GERP_CONTRACT_REQUIRED", "GERP_VENDOR_TYPE", "GERP_CD", "GERP_DIVISION", "GERP_COST_TYPE", "Last Legal Opinion"};
            keys = new String[]{"Registration Year", "Registration Month", "Registration Date", "NERP Subsidiary", "SELMS+ Subsidiary", "Contract ID", "Request Title", "Contract Name", "Requesting dept.", "ESBO dept.", "Requester (In Charge)", "Request Date", "Area of contract", "Contract Type", "Contract Matter", "Contract start date", "Contract end date", "Contract amount", "Currency Unit", "Counterparty", "Conclusion date", "Step", "Status", "Reviewer", "GERP_CUSTOMER_NM", "GERP_CONTRACT_REQUIRED", "GERP_VENDOR_TYPE", "GERP_CD", "GERP_DIVISION", "GERP_COST_TYPE", "Last Legal Opinion"};
        } else if ("l1".equals(queryCode)) {
            headers = new String[]{"Registration Year", "Registration Month", "Registration Date", "NERP Subsidiary", "SELMS+ Subsidiary", "Contract ID", "Request Title", "Contract Name", "Requesting dept.", "ESBO dept.", "Requester (In Charge)", "EMAIL", "Request Date", "Area of contract", "Contract Type", "Contract Matter", "Contract start date", "Contract end date", "Contract amount", "Currency Unit", "Conclusion date", "Step", "Status", "Counterparty", "Counterparty Code", "DP Agreement", "DPA Related Contract", "CCed"};
            keys = new String[]{"Registration Year", "Registration Month", "Registration Date", "NERP Subsidiary", "SELMS+ Subsidiary", "Contract ID", "Request Title", "Contract Name", "Requesting dept.", "ESBO dept.", "Requester (In Charge)", "EMAIL", "Request Date", "Area of contract", "Contract Type", "Contract Matter", "Contract start date", "Contract end date", "Contract amount", "Currency Unit", "Conclusion date", "Step", "Status", "Counterparty", "Counterparty Code", "DP Agreement", "DPA Related Contract", "CCed"};
            ...
            headers = new String[]{"Registration Year", "Registration Date", "NERP Subsidiary", "Request Title",
                    ...
                    "DP Agreement", "DPA Related Contract", "Area of contract", "CCed"};
