			...
			if(confirm("<spring:message code='secfw.msg.ask.insert' />")){
				if(frm.srch_rule_choice.value == "up"){//상위 selectbox 최상위 선택 시 null 체크.
				    if(frm.one_rule_title_input.value == ""){
				    	alert("<spring:message code='clm.msg.alert.regulation.upNoTitle' />");   // 상위 제목을 입력하세요.
				    	frm.one_rule_title_input.focus();
				    	return;
				    }	
				}
				
				if(frm.rule_title.value == ""){//제목  null 체크.
					alert("<spring:message code='clm.msg.alert.regulation.noTitle' />");   // 제목을 입력하세요.
					frm.rule_title.focus();
					return;
				}
				
				frm.method.value = "insertDimensionWordAdmin";
			    
			    //나모웹에디터 관련
			    //frm.rule_cont.value = frm.wec.MIMEValue;
			    //frm.rule_cont_en.value = frm.wec_en.MIMEValue;
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table-style01">
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3">
							<input type="text" name="rule_title" id="rule_title" value="<c:out value='${command.rule_title}'/>" required alt="<spring:message code="clm.page.field.regulation.ruleTitle" />" maxLength="100" class="text_full"/>
						</td>
					</tr>
<!-- 					<tr> -->
