...
<%}%>
	...
	$(document).ready(function(){
		...
		getCodeSelectByUpCd("frm", "rd_trgt2", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=CLM&combo_abbr=&combo_grp_cd=C057&combo_type=S&combo_del_yn=N&combo_selected=<%=lom.get("rd_trgt2")%>');
	...
	*/
	function pageAction(flag){
		...
		//저장
		if(flag == "insert"){
		    frm.method.value = "modifyBoard";
		    
		    //나모웹에디터 관련
		    frm.cont.value = frm.wec[0].MIMEValue;
		    frm.cont_en.value = frm.wec[1].MIMEValue;
			//공지대상1 체크
...
<div id="wrap">
	...
	<div id="container">	
		...
		<!-- title -->
		<div class="title">
			<h3><spring:message code="clm.page.title.board.modifyTitle" /></h3>
		</div>
		<!-- //title -->
		<!-- content -->
		<div id="content">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table-style01">
				...
				<tbody>
					<tr>
						...
						<td colspan="3">
							<input type="text" name="title" id="title" required alt="<spring:message code="clm.page.field.board.titleKr" />" maxLength="128" class="text_full" style="width:90%" value="<c:out value='${lom.title}' />" />
						</td>
					</tr>
					<!-- 영문공지사항 추가 -->
					<tr>
						...
						<td colspan="3">
							<input type="text" name="title_en" id="title_en" required alt="<spring:message code="clm.page.field.board.titleEn" />" maxLength="128" class="text_full" style="width:90%" value="<c:out value='${lom.title_en}' />" />
						</td>
					</tr>
					<tr>
					<tr>
					...
					<tr>
						...
						<td>
							<input type="radio" name="sec_yn" value="Y" <%if("Y".equals((String)lom.get("sec_yn"))){%>checked<%}%> ><spring:message code="clm.page.field.board.secYn1" /></input>
							<input type="radio" name="sec_yn" value="N" <%if("N".equals((String)lom.get("sec_yn"))){%>checked<%}%> ><spring:message code="clm.page.field.board.secYn2" /></input>
						</td>						
					</tr>
					<tr>
						...
						<td>
							<select name="annce_gbn" id="annce_gbn">
								<option value="T" <%if("".equals((String)lom.get("annce_startday")) && "".equals((String)lom.get("annce_endday"))){%>selected<%} %>><spring:message code="clm.page.field.board.annceGbn1" /></option>
								<option value="D" <%if(!"".equals((String)lom.get("annce_startday")) || !"".equals((String)lom.get("annce_endday"))){%>selected<%} %>><spring:message code="clm.page.field.board.annceGbn2" /></option>							
							</select>
						</td>						
						<th><spring:message code="clm.page.field.board.annceDt" /></th>
						<td>
							<span id="annce_dt" style="display:none">
							<input type="text" name="annce_startday" id="annce_startday" value="<%=DateUtil.formatDate((String)lom.get("annce_startday"), "-")%>"  class="text_calendar02"/>
							~
							<input type="text" name="annce_endday" id="annce_endday" value="<%=DateUtil.formatDate((String)lom.get("annce_endday"), "-")%>" class="text_calendar02"/>
							</span>
						</td>
					</tr>
					<tr>
...
<SCRIPT language="javascript" FOR="wec" EVENT="OnInitCompleted()">
	...
	document.wec[0].Value = document.frm.cont.value;
	...
	document.wec[1].Value = document.frm.cont_en.value;
</SCRIPT>
</html>
