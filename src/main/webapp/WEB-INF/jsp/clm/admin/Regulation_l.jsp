...
<script language="javascript">
	...
	$(document).ready(function(){
		getCodeSelectByUpCd("frm", "srch_two_rule_depth", "/clm/admin/regulation.do?method=getRuldCdHTML&combo_up_cd=<c:out value='${command.srch_one_rule_depth}'/>&combo_selected=<c:out value='${command.srch_two_rule_depth}'/>");
	});
	...
	$(function() {
		...
		$("#srch_one_rule_depth").change(function() {
			...
			getCodeSelectByUpCd("frm", "srch_two_rule_depth", "/clm/admin/regulation.do?method=getRuldCdHTML&combo_up_cd="+oneCd+"&combo_selected=");
		});
	...
	*/
	...
	function pageAction(flag){
		...
		if(flag == "search"){//조회.
			...
			frm.method.value = "forwardInsertRegulation";
			frm.submit();
		}
	}
	...
	*/
	function detailView(ruleNo){
		...
		frm.target = "_self";
		frm.action = "<c:url value='/clm/admin/regulation.do' />";
		frm.method.value = "detailRegulation";
							...
							<td>
								<table class="search_form">
									...
									<tr>										
								    	...
								    	<td colspan="3">
											<input type="text" id="srch_cont" class="text" name="srch_cont" style="width:300px" value="<c:out value='${command.srch_cont}'/>" />
										</td>
									</tr>
								</table>
							</td>
				...
				<colgroup>
			  	...
			  	<thead>
					<tr>
					    ...
					    <th><spring:message code="clm.page.field.regulation.regDt" /></th>
					</tr>
				</thead>
			 	<tbody>
			 	    <c:choose>
...
<%-- 					 	        <c:if test="${command.session_user_locale=='ko'}"> --%>
<%-- 					 	        	<td><c:out value="${list.one_title}"/></td> --%>
<%-- 					 	        	<td><c:out value="${list.two_title}"/></td> --%>
<%-- 					 	        </c:if> --%>
<%-- 					 	        <c:if test="${command.session_user_locale=='en'}"> --%>
									<td class="tL overflow" title="<c:out value="${list.one_title}"/>">
							 	        <c:out value="${list.one_title}"/>
						 	        </td>
						 	        <td class="tL overflow" title="<c:out value="${list.two_title_en}"/>">
							 	        <c:out value="${list.two_title_en}"/>
						 	        </td>
<%-- 					 	        </c:if> --%>
					 	        
					 	        <td class="tC overflow" title="<c:out value="${list.reg_nm}"/>">
						 	        <c:out value="${list.reg_nm}"/>
					 	        </td>
