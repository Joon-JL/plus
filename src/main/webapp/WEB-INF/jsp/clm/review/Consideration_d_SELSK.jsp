...
<%--
/**
 * 파  일  명 : Consideration_d_SELSK.jsp
...
<%
	...
	String cntrt_no = "" + lomRq.get("cntrt_no");
%>

<html>
<!-- page name: review/Consideration_d_SELSK.jsp -->
<head>
<meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"/>
<meta http-equiv="Pragma" content="no-cache"/>
<meta http-equiv="Expires" content="0"/>
<meta sci="Consideration_d_SELSK.jsp" />
<meta charset="utf-8" />
...
<script language="javascript" src="<c:url value='/script/secfw/jquery/calendar/calendar_clms_ko.js' />"></script>
...
<script language="javascript">
	...
	var HQ_ECMS_contract_status = null;
	var HQ_ECMS_contract_status_DESC = null;
	var HQ_ECMS_hq_reply = null;
	
	var contextPath = '<%=request.getContextPath()%>';
	if (!contextPath || contextPath === '') {
		//contextPath = 'http://selmsplus.sec.samsung.net';
	}
	var urlService = contextPath + '/api/ecms/ParallelReviewContractInquiryService';
	
	<%-- $.ajax({
	    url: urlService,
	    type: 'GET',
	    data: { comp_cd: '<%=comp_cd%>', cntrt_no: '<%=cntrt_no%>' },
	    cache: false,
	    async: false,
	    dataType: 'json',
	...
	if (responseObj != null) {
		if (responseObj.message == "ok") {
			HQ_ECMS_contract_status = responseObj.contract_status;
			HQ_ECMS_hq_reply = responseObj.hq_reply;
			
			//HQ_ECMS_contract_status_DESC = HQ_ECMS_contract_status;
			switch(HQ_ECMS_contract_status) {
				case "0": { HQ_ECMS_contract_status_DESC = "Not Sent" 						; break; } 
				case "1": { HQ_ECMS_contract_status_DESC = "Sent successfully (1)" 			; break; } 
				case "2": { HQ_ECMS_contract_status_DESC = "Sent successfully (2)" 			; break; } 
				case "3": { HQ_ECMS_contract_status_DESC = "Sent successfully (3)" 			; break; } 
				case "4": { HQ_ECMS_contract_status_DESC = "Replied - Check HQ answer!"		; break; } 
				case "5": { HQ_ECMS_contract_status_DESC = "Replied - OK" 					; break; }
				default:  { HQ_ECMS_contract_status_DESC = "ERROR - Contact System Admin" 	; break; }
			}
		}
		else {
			alert("Error(2): " + responseObj.message);
			goHome();
		}
	} --%>
...
<script language="javascript">
	...
	$(document).ready(function(){
		...
		//버튼 권한 적용 (2013.04 jnam)
		//top_role -> [RA01:법무관리자], [RA02:법무검토자], [RB01:전자검토자]
		//1. 1차 의뢰 배정전 [검토요청 상태] 
		...
		/*
		// 2014-11-21 Kevin commented. 있어서는 안되는 버튼임. 변호사가 다른 변호사를 어사인 할 수 없음. 본사에서 사용하는 기능이 그대로 소스 카피 된 것으로 보임. 아님 말구.
	...
	function namoView(){
		//1. 검토요청내용
		if(document.getElementById('if_cnsd_demnd_cont') != null){
	...
	*/
	function forwardConsideration(arg){
		...
		if(arg == "SAVE"  || arg == "SAVE2"  || arg =="DELIVERY" || arg =="DISPATCH" || arg == "NOAPPROVAL"){		// 임시저장, 의견전달, 발신, 결재없이 발신
			
           for(var i =1;i<100;i++){
				
				/*ch_Click = $("input[id=hqChk"+i+"]:checked").val();
							
				if(undefined == ch_Click){
					alert("<spring:message code="las.page.field.hqrequest.page01" /> ");
					return;
				}*/
				
        	    var $checkbox = $("#hqChk" + i);
			...
			if("<c:out value='${command.top_role}' />" == "RA01"){
				cnsd_opnn = CrossEditor.GetBodyValue() || '';
			} else {
				cnsd_opnn = CrossEditor1.GetBodyValue() || '';
			} 
			var escapeFormTags = cnsd_opnn.replace(escapeFormReg,"");
			 
			 
			 
			$('#frm #cnsd_opnn').val(escapeFormTags);
			 
			 
			//Sungwoo added Check_list_yn 여부 추가 2014-12-01
			//Brice block check_list_yn by request of Shorn 2019-04-19
			/*
			if(frm.check_list_yn.value == ""
				&& "<c:out value='${lomRq.plndbn_req_yn}'/>" == "Y"
				&& "<c:out value='${lomRq.biz_cd }'/>" == "T0103"
				&& "<c:out value='${lomRq.depth_cd}'/>" == "T0201"
				&& ("<c:out value='${lomRq.cnsl_mid_cd3}'/>" == "T030705" || "<c:out value='${lomRq.cnsl_mid_cd3}'/>" == "T030706" )
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030705002"
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030705003"
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030705006"
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030706002"
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030706003"
				&& "<c:out value='${lomRq.cntrt_trgt_cd}'/>" != "T030706006"
			){
				alert("<spring:message code='las.page.field.hqrequest.page11' />");
				return;
			}
			*/
			returnConsideration(arg);
			...
			var escapeFormTags = (CrossEditor.GetBodyValue() || '').replace(escapeFormReg,"");
			//var escapeFormTags = cnsd_opnn.replace(escapeFormReg,"");
			
			//frm.cnsd_opnn.value = escapeFormTags.html(); //  종합검토의견
			$('#cnsd_opnn').val(escapeFormTags);
			approvalOpnn();
			return;
			...
			if( "C04223" == prgrs_status && "C02686" == depth_status && "C04305" == cnsd_status ){
				frm.target = "_self";		 
				frm.action = "<c:url value='/clm/manage/myContract.do' />";
				frm.method.value = "listMyContract";
				frm.submit();
			} else {
				// 기존 검토이관으로 으로 이동 시키게 됩니다.	
				frm.target = "_self";		 
				frm.action = "<c:url value='/clm/review/consideration.do' />";
			...
			// 담당자와 로그인 사용자가 다를 경우에는 바로 리턴.
			var isAssignedReviewer = false;
			$("input[name='list_respman_ids']").each(function(idx, ele){
					if($(ele).val() == "<%=userId%>"){
			...
			if(!isAssignedReviewer){	
				alert("<spring:message code='las.msg.alert.notAllowedAction' />");
				return;
			}
			else{
				$("#page_div").val("RESP");
		...
		else if(arg == "HQ"){		// 본사 검토 요청
			
			var str_msg = "<spring:message code='las.page.field.hqrequest.page14' />";
		...
		} else if(arg == "REHQ"){		// 본사 검토 요청
			
			var str_msg = "<spring:message code='las.page.field.hqrequest.page15' />";
			...
			for(var i =1;i<10;i++){
				...
				if(undefined == ch_Click){
					alert("<spring:message code='las.page.field.hqrequest.page16' />");
					return;
				}
			}
			
			//var str_msg = "It is important to save your document before sending to SWON Parallel Review. Confirm?";
			var str_msg = "Would like to send a notification to SUWON ?";
			if(!confirm(str_msg)){
				return;
			}
			forwardConsideration("SAVE");
			
			var sendToHqFunction = function() {
			
				//var str_msg = "Would like to send a notification to SUWON ?";			
				//if(confirm(str_msg)){
					
					//var hq_req_title = "Sending notification to SUWON";
					//PopUpWindowOpen2("/clm/review/consideration.do?method=notificationToHQ&menu_id=20130319152827188_0000000335&cntrt_id="+frm.cntrt_id.value+"&cnsdreq_id="+frm.cnsdreq_id.value+"&hq_req_title="+hq_req_title+"&prev_cnsdreq_id="+frm.prev_cnsdreq_id.value, 800, 400, false, "HQrequestWindows");
					//forwardRequestHQ
					...
					var urlService = contextPath + '/api/ecms/ParallelReviewRequestService';
					...
					$.ajax({
			            ...
			            success: function (response) {
			            	if (response != null && response.message != "ok") {
		            			alert("Error(3): " + response.message);
			            	}
	...
	*/
	function returnConsideration(arg){
		...
		var options = {
			...
			success: function(responseText, statusText) {
				...
				if(arg=="SAVE")
				{
					window.setTimeout(function() {forwardConsideration("LIST");},5000);
				}
	...
	function frmSubmit(options,arg){
		...
		<%if( ("RA01".equals(command.getTop_role()) || "RB01".equals(command.getTop_role())) && lomRq.get("mn_cnsd_dept").equals(command.getBlngt_orgnz())){%>
			...
			}else if((arg == "SAVE")||(arg == "SAVE2")){		// 임시저장
			...
			if($('#plndbn_req_yn').val() == "Y" && $("#blngt_orgnz").val() == $("#mn_cnsd_dept").val() && $("#respman_apnt_yn").val() == "Y" && $("#btn_div1").val() == "Y"){				
				...
				fileListReContract.UploadFile(function(uploadCount){
					//Sungwoo added 2014-09-29 reAttach valudation 처리 위해 변경
					if( true && uploadCount == 0 && $('#fileInfosReContract')[0].value == '' // change "true" to "false" for testing in case NamoEditor is bugged (FERNANDO) 
						&& (arg == "DISPATCH" || arg == "RESP" || arg=="NOAPPROVAL")
					){
						viewHiddenProgress(false);
		...
		<%}else if( "RA02".equals(command.getTop_role()) && lomRq.get("mn_cnsd_dept").equals(command.getBlngt_orgnz())) { //로그인자가 정검토부서 이고  담당자인경우 InserTmode%>
			if( "Y" == $('#plndbn_req_yn').val() && ( "SAVE" == arg || "SAVE2" == arg || "DISPATCH" == arg || "TABSAVE" == arg ) ){ //정검토 부서 이며 담당자 최종본이면서 임시저장 발신시 자동갱신여 체크해야 함								
			...
			if(arg =="DELIVERY" || arg =="DISPATCH" || arg == "RESP" || arg == "NOAPPROVAL"){
				if($("#top_role").val() == "RA02" ) {
					...
					if("<c:out value='${lomRq.plndbn_req_yn}'/>" == "Y"){ //최종본
						// 2014-05-29 Kevin added.
						if($("#cntrtperiod_startday").val() == ""){
							alert("<spring:message code='las.page.field.main.invalidContractStartDate' />");
							$("#cntrtperiod_startday").focus();
							return false;
						}
						if($("#cntrtperiod_endday").val() == ""){
							alert("<spring:message code='las.page.field.main.invalidContractEndDate' />");
							$("#cntrtperiod_endday").focus();
							return false;
						}
						var str_from = $("#cntrtperiod_startday").val().replace('-','').replace('-','');
						var str_to = $("#cntrtperiod_endday").val().replace('-','').replace('-','');
					...
					//검토의견
					if( true && (stripHTMLtag(frm.body_mime_rq.value) == "")){ // change "true" to "false" for testing in case NamoEditor is bugged (FERNANDO)
						alert("<spring:message code='las.page.field.main.confirmValue' arguments='"+msgVal+"' />");
						return;
					}
				}
	...
	function initPage(){
		...
		// 2014-08-18 Kevin.
		//fileListContract, fileListEtc, fileListGita, fileListOL 삭제됨.
		...
		contractHisList();	//Sungwoo added 2014-09-09

		//Sungwoo 2014-09-23 Replaced status. 공통으로 정리
		var gbn_status = "download";
		<% if(
				//Top role. Legal Admin or Reviewer 만 수정권한을 가진다.
				("RA01".equals(command.getTop_role()) || "RB01".equals(command.getTop_role()) || "RA02".equals(command.getTop_role()))
				//Not Replied, Draft saved status
				&& ("C04301".equals(lomMn.get("cnsd_status")) || "C04302".equals(lomMn.get("cnsd_status")) || lomMn.get("cnsd_status") == null)
			){ %>
			gbn_status = "modify";
		<% }%>
		
		<% 
		//계약 보류상태에서는 Reviewer의 첨부파일내역이 나타나지 않아 조건 처리
		if((lomRq.get("prgrs_status").equals("C04204") && request.getAttribute("respYn").equals("Y"))	//Review in Progress 상태인경우, 첨부파일이 없으며 respYn(수정권한이 있는경우) 나타남
				|| lomRq.get("prgrs_status").equals("C04207") //Replied 일 경우 
			){
		%>
			// 회신 - 정_계약서
	...
	function implJson(method){
		...
		if(method == "approvalOpnn"){
			...
			if("RA01" == $("#top_role").val() || "RB01" == $("#top_role").val()){				// 그룹장 
				...
				//필수체크 추가(문서모드 IE9)
				if( true && (stripHTMLtag(frm.body_mime_rq.value) == "" || stripHTMLtag(frm.body_mime_rq.value) == "&nbsp;")){ // change "true" to "false" for testing in case NamoEditor is bugged (FERNANDO)
	...
	*/
	function approvalOpnn() {
		...
		if(confirm(msg) && ( false || validateTempSave(frm))){ // change "false" to "true" for testing in case NamoEditor is bugged (FERNANDO) 
			if($("#top_role").val() == "RA01" || $("#top_role").val() == "RB01"){
	...
	*/
	function goLastConsideration(gbn_last){
		...
		var frm = document.frm;	
		frm.plndbn_req_yn.value = gbn_last;					// 최종본 회신구분
	...
	* CLOSE 처리 Popup
	*/
	function popCloseReq(){
		var frm = document.frm;
		
		PopUpWindowOpen('', 600, 200, true);
		frm.method.value = "popCloseReq";
		frm.action = "<c:url value='/clm/review/consideration.do' />",
		frm.target = "PopUpWindow";
	...
	// 추가 검토의견을 넣을 수 있는 팝업창 띄우기
	function forwardLegalOpinion(){
		var frm = document.frm;
		PopUpWindowOpen2("/clm/review/consideration.do?method=forwardLegalOpinion&menu_id=20140225032722002_0000000462&cntrt_id="+frm.cntrt_id.value+"&con_depth_status="+frm.con_depth_status.value , 1000, 600, false, "PopUpLegalOpinion");
	}
	
	function allReqAtt(){
		if($('#allAttChk')[0].checked){
			$('#reqContChk')[0].checked = true;
			$('#reqAttChk')[0].checked = true;
			$('#otherAttChk')[0].checked = true;
		}else{
			$('#reqContChk')[0].checked = false;
			$('#reqAttChk')[0].checked = false;
			$('#otherAttChk')[0].checked = false;
		}
	...
	function reqContAtt(){
		...
		if($('#reqContChk')[0].checked){
			frm.view_gbn.value = "reAttach";
	...
	function showHQbtn(){
		...
		var top30 = frm.db_top30.value;
		var cont_type = frm.cont_type_cd.value;
		var hq_cnsd_status  = frm.hq_cnsd_status.value;

		//결재없이회신버튼 제어를 위하여 추가(Sungwoo added 2014-11-17)
		var plndbn_req_yn			= $('#plndbn_req_yn').val();							//최종본의뢰여부
		...
		for(var i =1;i<100;i++){
    	    ...
    	    if ($checkbox.length !== 0) {
				...
				ch_Click = $("input[id=hqChk"+i+"]:checked").val();
				ch_Click_key = $("input[name=cd_nm"+i+"]").val();
				
				sum_ch_Click += ch_Click+","+ch_Click_key+";";
		...
		// 30대 거래선인지 체크 하기
		// TN_CLM_CONTRACT_MASTER에서 30대 거래선 여부 가지고 오기
		var top30 = frm.db_top30.value;
		// 계약 유형 확인
		// TN_CLM_CONTRACT_MASTER에서 유형 가지고 오기
		var cont_type = frm.cont_type_cd.value;
		// 계약서 종류 구분 (자기꺼 남의것)   SOC30 이 코드 일 경우에만 체크
		var cont_kind = frm.cont_kind_type.value;
		
		// hq 병행 검토 요청 했는지 여부 
		var hq_req_yn = frm.hq_req_yn.value;
		
		//Formal Contract
		var db_biz_cd = frm.db_biz_cd.value;
		
		//New contract
		var db_depth_cd = frm.db_depth_cd.value;
		if((false && ("T0103" == db_biz_cd) &&("T0201" == db_depth_cd) && ("T030705" == cont_type || "T030706" == cont_type) && "SOC30" == cont_kind ) && "" == hq_cnsd_status) {
			$("#btn_up20").show();						// 상단 REQUEST TO HQ
			$("#btn_down20").show();				    // 하단 REQUEST TO HQ
			$("#btn_up21").hide();						// 상단 REQUEST TO HQ
			$("#btn_down21").hide();				    // 하단 REQUEST TO HQ
			$("#btn_up3").hide();						// 상단 발신
			$("#btn_down3").hide();                     // 하단 발신
			$("#btn_up_lastConsideration").hide();    // 상단 최종본 회신
			$("#btn_down_lastConsideration").hide();  // 하단 최종본 회신
		} else {
			if("C16201" == hq_cnsd_status || "" == hq_cnsd_status){
				...
				//false 초기엔 false
					...
					$("#btn_up20").hide();						// 상단 REQUEST TO HQ
					$("#btn_down20").hide();				    // 하단 REQUEST TO HQ
					$("#btn_up21").hide();						// 상단 REQUEST TO HQ
					$("#btn_down21").hide();				    // 하단 REQUEST TO HQ
					$("#btn_up3").show();						// 상단 발신
					$("#btn_down3").show();                     // 하단 발신
					
					if(plndbn_req_yn != 'Y'){
						$("#btn_up_lastConsideration").show();    // 상단 최종본 회신
						$("#btn_down_lastConsideration").show();  // 하단 최종본 회신
					}
		...
		frm.hqreqcheck.value = sum_ch_Click;
		
		// Fernando - checking buttons according to SUWON Parallel Review status
		
		// btn_up20 // REQUEST TO SUWON
		// btn_up1  // SAVE
		// btn_up_modifyContract // Modify Requested Information button
		// btn_up3 // Reply
		// btn_down_lastConsideration // Reply as Final Version
		/* 
		if (HQ_ECMS_contract_status == "1" ||
			HQ_ECMS_contract_status == "2" ||
			HQ_ECMS_contract_status == "3" ||
			HQ_ECMS_contract_status == "x" ) 
		{
			$("#btn_up20").hide();
			$("#btn_up1").hide();
			$("#btn_up_modifyContract").hide();
			$("#btn_up3").hide();
			$("#btn_up_lastConsideration").hide();
			
			$("#btn_down20").hide();
			$("#btn_down1").hide();
			$("#btn_down_modifyContract").hide();
			$("#btn_down3").hide();
			$("#btn_down_lastConsideration").hide();
		}
		
		if (HQ_ECMS_contract_status == "4" ||
			HQ_ECMS_contract_status == "x" ) 
		{
			$("#btn_up20").show();
			$("#btn_up1").show();
	...
	function chkReqHQContractType(){
		...
		if((false && ("T0103" == db_biz_cd) &&("T0201" == db_depth_cd) && ("T030705" == cont_type || "T030706" == cont_type) && "SOC30" == cont_kind ) && "" == hq_cnsd_status) {
			if("" != prev_cnsdreq_id || cnsdreq_id != prev_cnsdreq_id){
				if("N" == hq_req_yn){
					$("#btn_up20").show();						// 상단 REQUEST TO HQ
					$("#btn_down20").show();				    // 하단 REQUEST TO HQ
					$("#btn_up21").hide();						// 상단 REQUEST TO HQ
					...
					$("#btn_up20").hide();						// 상단 REQUEST TO HQ
					$("#btn_down20").hide();				    // 하단 REQUEST TO HQ
					$("#btn_up21").show();						// 상단 REQUEST TO HQ
					$("#btn_down21").show();				    // 하단 REQUEST TO HQ
					$("#btn_up3").show();						// 상단 발신
					$("#btn_down3").show();                     // 하단 발신
					$("#btn_up_lastConsideration").show();    // 상단 최종본 회신
					$("#btn_down_lastConsideration").show();  // 하단 최종본 회신
		...
		} else {   // 이 후의 hq의뢰 이후 상태 값으로 체크 			
			if("C16201" == hq_cnsd_status){
				$("#btn_up20").show();						// 상단 REQUEST TO HQ
				$("#btn_down20").show();				    // 하단 REQUEST TO HQ
				$("#btn_up21").hide();						// 상단 REQUEST TO HQ
				$("#btn_down21").hide();				    // 하단 REQUEST TO HQ
				...
				$("#btn_up_lastConsideration").hide();    // 상단 최종본 회신
				$("#btn_down_lastConsideration").hide();  // 하단 최종본 회신
				$("#btn_up1").show();  // 상단 SAVA 버튼
				$("#btn_down1").show();  // 하단 SAVA 버튼
			} else if("C16205" == hq_cnsd_status || "C16207" == hq_cnsd_status){
				$("#btn_up20").hide();						// 상단 REQUEST TO HQ
				$("#btn_down20").hide();				    // 하단 REQUEST TO HQ
				$("#btn_up21").show();						// 상단 REQUEST TO HQ
				$("#btn_down21").show();				    // 하단 REQUEST TO HQ
				...
				$("#btn_up_lastConsideration").show();    // 상단 최종본 회신
				$("#btn_down_lastConsideration").show();  // 하단 최종본 회신
			} else if("C16202" == hq_cnsd_status || "C16203" == hq_cnsd_status || "C16204" == hq_cnsd_status || "C16206" == hq_cnsd_status){
				$("#btn_up20").hide();						// 상단 REQUEST TO HQ
				$("#btn_down20").hide();				    // 하단 REQUEST TO HQ
				$("#btn_up21").hide();						// 상단 REQUEST TO HQ
				$("#btn_down21").hide();				    // 하단 REQUEST TO HQ
				...
				$("#btn_up_lastConsideration").hide();    // 상단 최종본 회신
				$("#btn_down_lastConsideration").hide();  // 하단 최종본 회신
				$("#btn_up1").hide();  // 상단 SAVA 버튼
				$("#btn_down1").hide();  // 하단 SAVA 버튼
				$("#btn_up_modifyContract").hide();  // 상단 계약 수정 버튼
				$("#btn_down_modifyContract").hide();  // 하단 계약 수정 버튼
			} else if("" == hq_cnsd_status ){
                if("Y" == hq_req_yn){
                	$("#btn_up20").hide();						// 상단 REQUEST TO HQ
    				$("#btn_down20").hide();				    // 하단 REQUEST TO HQ
    				$("#btn_up21").show();						// 상단 REQUEST TO HQ
		...
		// 추가 요청 : legal 이 없는 계약 유형의 경우 request to hq 버튼이 보이면 안됨.
		<c:choose>    
		    <c:when test="${ ('T030705006' eq lomRq.cntrt_trgt_cd || 'T030705003' eq lomRq.cntrt_trgt_cd || 'T030705002' eq lomRq.cntrt_trgt_cd || 'T030706006' eq lomRq.cntrt_trgt_cd || 'T030706003' eq lomRq.cntrt_trgt_cd || 'T030706002' eq lomRq.cntrt_trgt_cd || 'T030706999' eq lomRq.cntrt_trgt_cd || 'T030705999' eq lomRq.cntrt_trgt_cd   )  }">
		        $("#btn_up20").hide();						// 상단 REQUEST TO HQ
		        $("#btn_down20").hide();				    // 하단 REQUEST TO HQ
		        $("#btn_up21").hide();						// 상단 REQUEST TO HQ
		        $("#btn_down21").hide();				    // 하단 REQUEST TO HQ
	...
	function reqToHQ(){
		...
		var hq_req_title = "[<c:out value='${command.comp_cd}'/>]"+ " <c:out value='${lomRq.req_title}' escapeXml='false'/>";
		 
	  // 검토 의뢰를 할 수 있는 팝업을 연다.
	  PopUpWindowOpen2("/clm/review/consideration.do?method=forwardRequestHQ&menu_id=20130319152827188_0000000335&cntrt_id="+frm.cntrt_id.value+"&cnsdreq_id="+frm.cnsdreq_id.value+"&hq_req_title="+hq_req_title+"&prev_cnsdreq_id="+frm.prev_cnsdreq_id.value, 800, 400, false, "HQrequestWindows");
	}
	
	/**
	 * HQ 검토 재검토 의뢰 처리 - 팝업으로 처리 됩니다.
	 */
	function reqToREHQ(){
	  var hq_req_title = "[<c:out value='${command.comp_cd}'/>]"+ " <c:out value='${lomRq.req_title}' escapeXml='false'/>";	
	  // 검토 재 의뢰를 할 수 있는 팝업을 연다.
	  PopUpWindowOpen2("/clm/review/consideration.do?method=forwardRERequestHQ&menu_id=20130319152827188_0000000335&cntrt_id="+frm.cntrt_id.value+"&cnsdreq_id="+frm.cnsdreq_id.value+"&hq_req_title="+hq_req_title+"&prev_cnsdreq_id="+frm.prev_cnsdreq_id.value, 800, 400, false, "HQrequestWindows");
	}
	...
	*/
	...
	function checkListAppend(){
	    var options = {   
				...
				dataType: "json",
				success: function(responseText, statusText) {
					frm.check_list_yn.value = "Y";		//Sungwoo added for validation check 2014-12-03
	...
	function setCheckList(){
		...
		<c:choose>
	     <c:when test="${ 'T0103' eq lomRq.biz_cd && 'T0201' eq lomRq.depth_cd && ('T030705' eq lomRq.cnsl_mid_cd3 || 'T030706' eq lomRq.cnsl_mid_cd3 )  }">
	         <c:choose>    
	             <c:when test="${ ('T030705006' eq lomRq.cntrt_trgt_cd || 'T030705003' eq lomRq.cntrt_trgt_cd || 'T030705002' eq lomRq.cntrt_trgt_cd || 'T030706006' eq lomRq.cntrt_trgt_cd || 'T030706003' eq lomRq.cntrt_trgt_cd || 'T030706002' eq lomRq.cntrt_trgt_cd   )  }"> 
      ...
      <c:choose>
	    <c:when test='${0 < chekItemListSize }'>
	        <c:forEach var="chekList" items="${chekItemList }">
		        ...
		        + "    <td><c:out value="${chekList.remark }" escapeXml="true"/></td>"
		        + "</tr>"
	        </c:forEach>
	    </c:when>
	    <c:otherwise>
		...
		<div id="content">
			...
			<div id="content_in">				
				<form:form name="frm" id='frm' method="post" autocomplete="off">
					...
					<input type="hidden" name="srch_cntrt_no"		value="<c:out value='${command.srch_cntrt_no}' escapeXml='false'/>" />
					...
					<!-- 30대 거래선 및 계약 유형, 계약서 사용한 내용 -->
					<input type="hidden" name="db_top30" id="db_top30"  value="<c:out value='${lomRq.top_30_cus}'/>" />
					<input type="hidden" name="cont_type_cd" id="cont_type_cd"  value="<c:out value='${lomRq.cnsl_mid_cd3}'/>" />
					<input type="hidden" name="cont_kind_type" id="cont_kind_type"  value="<c:out value='${lomRq.cont_kind_type}'/>" />
					<input type="hidden" name="hq_req_yn" id="hq_req_yn"  value="<c:out value='${lomRq.hq_req_yn}'/>" />
					<input type="hidden" name="db_biz_cd" id="db_biz_cd"  value="<c:out value='${lomRq.biz_cd}'/>" />
					<input type="hidden" name="db_depth_cd" id="db_depth_cd"  value="<c:out value='${lomRq.depth_cd}'/>" />
					<input type="hidden" name="openCheckList" id="openCheckList"  value="" />
					...
					<input type="hidden" name="checkReqItem" /> <!-- 체크창을 띄우고 내용을 저장했는지 판단 함. -->		
					<!-- 관련자 데이타 설정  -->					
					<input type="hidden" name="client_modify_div" id="client_modify_div" />
					<input type="hidden" name="chose_client" id="chose_client" />
					<!-- hq 병행 체크 값을 넘기기 위한 값. -->
					<input type="hidden" name="hqreqcheck" id="hqreqcheck" />
					<input type="hidden" name="hq_req_title" id="hq_req_title" />   <!-- 의뢰 팝업 창에 제목을 전달 -->
					<!-- hq 병행 검토 상태 값 -->
					<input type="hidden" name="hq_cnsd_status" id="hq_cnsd_status" value="<c:out value='${command.hq_cnsd_status}'/>" />	
					<!-- 관련자 데이타 설정  -->					
					<input type="hidden" name="chose_client" id="chose_client" />
					<!-- Check list yn 필드 값 상태 추가 -->
					...
					<div class="btnwrap mt_22">
						<c:if test="${command.top_role == 'RA01' && (lomRq.depth_status != 'C02607' && lomRq.depth_status != 'C02603' && lomRq.depth_status != 'C02609') && (lomRq.prgrs_status == 'C04202' || lomRq.prgrs_status == 'C04203' || lomRq.prgrs_status == 'C04204'  ) && (empty lomResp) }"><!-- 최초 배정전 상태 -->
							<span id="btn_up15"  class="btn_all_w" onclick="popReturn();"><span class="mail"></span><a><spring:message code="las.filed.considetration.adminreply"/></a></span><!-- Admin Reply -->
						</c:if>
						<c:if test="${command.top_role == 'RA01' && (lomRq.CLOSE_YN eq 'N' || empty lomRq.CLOSE_YN) }"><!-- CLOSE 되어있지 않은 상태 -->
							<span id="btn_up16"  class="btn_all_w" onclick="popCloseReq();"><span class="end"></span><a><spring:message code="clm.page.button.contract.close"/></a></span><!--Close-->
						</c:if>
						...
						<!-- 발신 -->
						<span id="btn_up3" class="btn_all_w" style="display:none" onclick="forwardConsideration('DISPATCH');">
							<span class="mail"></span>
							<c:choose>
								<c:when test="${(lomRq.plndbn_req_yn == 'Y')}">
									<a><spring:message code="las.page.field.contractManager.sendFinal"/></a>
								</c:when>
								<c:otherwise>
									<a><spring:message code="las.page.field.contractManager.return"/></a>
								</c:otherwise>
							</c:choose>
						</span>
						<!-- HQ REQUEST 버튼 상단 -->
						<span id="btn_up20"disabled style="display:none"> <span  class="btn_all_w" onclick="forwardConsideration('EMAIL-TO-HQ');" ><span class="mail"></span><a>Request REVIEW to SUWON</a></span></span>
						<span id="btn_up20" style="display:none"> <span  class="btn_all_w" onclick="forwardConsideration('HQ');" ><span class="mail"></span><a><spring:message code="las.page.field.hqrequest.page17"/></a></span></span><!-- REQUEST TO HQ -->
						<span id="btn_up21" class="btn_all_w" style="display:none" onclick="forwardConsideration('REHQ');" ><span class="mail"></span><a><spring:message code="las.page.field.hqrequest.page18"/></a></span><!-- REQUEST TO HQ 재의뢰-->
						...
						<span id="btn_up_modifyContract" class="btn_all_w" style="display:none" onclick="forwardContractMaster();" ><span class="modify"></span><a><spring:message code="las.page.field.contractManager.requUpdate"/><!-- 의뢰내용 수정 --><span class='b_info' onMouseover="fixedtooltip('<spring:message code='las.page.field.contractManager.canUpate'/>', this, event, '300px')" onMouseout="delayhidetip()"></span></a></span>
						 <% if(authYN_RA02){ %>
							<span id="btn_up6" class="btn_all_w" style="display:none" onclick="forwardConsideration('CNSDRESP');" ><span class="reset"></span><a><c:choose><c:when test="${(lomRq.mn_cnsd_dept == command.blngt_orgnz && lomMn.cnsd_status == 'C04302')}"><spring:message code="las.page.field.contractManager.reviewUpdate"/><!--검토수정--></c:when><c:otherwise><spring:message code="las.page.field.contractManager.createReview"/><!--검토작성--></c:otherwise></c:choose></a></span>
						<%}%>
						 
						<c:if test="${(command.top_role == 'RA01' &&  lomRq.depth_status == 'C02609'  && lomRq.prgrs_status == 'C04226') || (command.top_role == 'RA02' &&   lomRq.depth_status == 'C02608' && lomRq.prgrs_status == 'C04207') }"><!-- 미체결 이면서 회신 상태 -->
							<span id="btn_up11" class="btn_all_w"  onclick="forwardLegalOpinion();" ><span class="preview"></span><a><spring:message code="las.page.field.lawconsulting.update"/> <spring:message code="las.page.field.statistics.return"/></a></span><!-- 추가 검토의견 김재원 버튼 추가 -->
						</c:if>
						<!--	2014-11-21 Kevin commented. 있어서는 안되는 버튼임. 변호사가 다른 변호사를 어사인 할 수 없음. 본사에서 사용하는 기능이 그대로 소스 카피 된 것으로 보임. 아님 말구.
						<span id="btn_up7" class="btn_all_w" style="display:none" onclick="forwardConsideration('CNSDAPPR');" ><span class="check2"></span><a><spring:message code="las.page.field.contractManager.pickPic"/></a></span>
						 -->
					...
					<!-- 2014-08-15 Kevin changed. Basic Information 섹션을 공통 페이지로 분리해서 include 시킨다. -->
					<jsp:include page="/WEB-INF/jsp/clm/common/basicInfo.jsp" flush="false">
						<jsp:param name="cnsdreq_id" value='<%=lomRq.get("cnsdreq_id") %>' />
						<jsp:param name="is_contactreview" value="Y" />
					</jsp:include>
				...
				<div id="tr_down02" style="display:none;">
					...
					<table cellspacing="0" cellpadding="0" class="table-style01">
						...
						<tr>
							...
							<tr>
								<th style="border-top:0px;"><spring:message code="las.page.field.contractmanager.consideration_inner_d.cntrtperiod" /><span class='astro'>*</span></th><!-- 계약기간  -->
								<td colspan="4">
									<!-- 2014-05-29 Kevin added. To make it editable -->
									<input type="text" name="cntrtperiod_startday" id="cntrtperiod_startday" required alt="<spring:message code="las.page.field.contractManager.contFromDate"/>" value='<%=DateUtil.formatDate((String)lomRq.get("cntrtperiod_startday"),"-")%>' class="text_calendar02"/> ~ 
									<input type="text" name="cntrtperiod_endday" id="cntrtperiod_endday" required alt="<spring:message code="las.page.field.contractManager.contToDate"/>" value='<%=DateUtil.formatDate((String)lomRq.get("cntrtperiod_endday"),"-")%>' class="text_calendar02" />
								</td>
							...
							<tr>
								...
								<td colspan="6">
									<input type="radio" name="auto_rnew_yn" value="Y" <%if("Y".equals(lomRq.get("auto_rnew_yn"))){out.println(" checked");} %> />Yes
									<input type="radio" name="auto_rnew_yn" value="N" <%if(!"Y".equals(lomRq.get("auto_rnew_yn"))){out.println(" checked");} %>/>No
								</td>
							</tr>
							<tr>
							<c:choose>
						...
						<table cellspacing="0" cellpadding="0" border="0" class="table-style01">
								...
								<c:when test ="${!empty relationList}">
									<c:forEach var="list" items="${relationList}" varStatus="i">
										<tr id="trRelationContractCont">
											...
											<td><a href="javascript:goReDetail('<c:out value='${list.cntrt_id}' />');"><c:out value='${list.relation_cntrt_nm}' escapeXml='false'/></a></td>
							...
							<c:otherwise>
								<tr id="trRelationContract">
									 ...
									 <td><input type="text" name="expl" id="expl" class="text_full" style="width:74%" onkeyup="f_chk_byte(this,600)" maxLength="600" /></td>
									 <td>
				...
				<!-- //연관계약정보 -->
				<!-- 본사 병행 검토 대상 계약 체크 시작 김재원 -->
				<c:if test="${command.top_role == 'RA02' && (lomRq.depth_status != 'C02607' && lomRq.depth_status != 'C02603') && (lomRq.prgrs_status == 'C04202' || lomRq.prgrs_status == 'C04203' || lomRq.prgrs_status == 'C04204' || lomRq.prgrs_status == 'C04206'  || lomRq.prgrs_status == 'C04207' )}">
				<c:choose>
					<c:when test="${lomRq.depth_status == 'C02606' && lomRq.prgrs_status == 'C04204'}">
				<div id="tr_down101">
					<div class="title_basic">
						<h4><spring:message code="las.page.field.hqrequest.page06" /><font color="red"> *</font> <img src="<%=IMAGE%>/btn/btn_up.gif" alt="up" onclick="tit_Toggle(this,'tr_down_check');" style='cursor:pointer;'/></h4>
					</div>
					<!-- 본사 병행 검토 대상 계약 체크 -->
					<div id="tr_down_check">
						<table id="tr_down_check" cellspacing="0" cellpadding="0" border="0" class="table-style01" >
							...
							<!-- 본사 병행 검토 대상 계약 체크  리스트 -->				
								<!-- <div id="div_contracts_check_list" class="title_basic3" >본사 병행 검토 대상 계약 체크<!-- 본사 병행 검토 대상 계약 체크  </div>--> 
									<tr>
									    <th class='tC'>No<!-- 번호 --></th>
									    <th class='tC'><spring:message code="las.page.field.hqrequest.page08" /><!-- 번호 --></th>
								        <th class='tC'><spring:message code="las.page.field.lawconsult.yes" /><!-- 체크 항목 --></th>
								        <th class='tC'><spring:message code="las.page.field.lawconsult.no2" /><!-- --></th>
									</tr>
									<c:choose>
									    <c:when test="${ 0 < hqchekItemListSize }">
									        <c:forEach var="hqchekItemList" items="${hqchekItemList}" varStatus="i">
									            ...
									            <c:choose>
									            <c:when test="${ 'Y' eq hqchekItemList.use_yn }">
										            <tr>
										                <td class='tC'><c:out value="${hqchekItemList.no }" escapeXml=""/></td>
										                <td><c:out value="${hqchekItemList.cd_nm }" escapeXml=""/></td>
										                <input type="hidden" name="cd_nm<c:out value="${hqchekItemList.no }"/>" value="<c:out value="${hqchekItemList.cd }"/>" />
										                <c:choose>
										                    <c:when test="${ 'Y' eq hqchekItemList.check_yn }">
										                        <td class='tC'><input type="radio" value='Y' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();" checked="checked"> <spring:message code="las.page.field.lawconsult.yes" /></td>
											                    <td class='tC'><input type="radio" value='N' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.no2" /></td>
										                    </c:when>
										                    <c:when test="${ 'N' eq hqchekItemList.check_yn }">
										                        <td class='tC'><input type="radio" value='Y' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.yes" /></td>
											                    <td class='tC'><input type="radio" value='N' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();" checked="checked"> <spring:message code="las.page.field.lawconsult.no2" /></td>
										                    </c:when>
										                    <c:otherwise>
										                        <!-- <td class='tC'><input type="radio" value='Y' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.yes" /></td>
											                    <td class='tC'><input type="radio" value='N' id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.no2" /></td> -->
						...
						<br/>
						<spring:message code="las.page.field.hqrequest.page07" />
					</div>
					<!--// 본사 병행 검토 대상 계약 체크  리스트 -->
				</div>
				</c:when>
									    ...
									    <c:when test="${ 0 < hqchekItemListSize }">
									        <c:forEach var="hqchekItemList" items="${hqchekItemList}" varStatus="i">
									            ...
									            <tr>
									                ...
									                <c:choose>
									                    <c:when test="${ 'Y' eq hqchekItemList.check_yn }">
									                        <td class='tC'><input type="radio" value='Y' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();" checked="checked"> <spring:message code="las.page.field.lawconsult.yes" /></td>
										                    <td class='tC'><input type="radio" value='N' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.no2" /></td>
									                    </c:when>
									                    <c:when test="${ 'N' eq hqchekItemList.check_yn }">
									                        <td class='tC'><input type="radio" value='Y' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.yes" /></td>
										                    <td class='tC'><input type="radio" value='N' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();" checked="checked"> <spring:message code="las.page.field.lawconsult.no2" /></td>
									                    </c:when>
									                    <c:otherwise>
									                        <!-- <td class='tC'><input type="radio" value='Y' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.yes" /></td>
										                    <td class='tC'><input type="radio" value='N' disabled="disabled" id="hqChk<c:out value="${hqchekItemList.no }"/>" name='<c:out value="${hqchekItemList.no }"/>' onclick="javascript:showHQbtn();"> <spring:message code="las.page.field.lawconsult.no2" /></td> -->
					...
					<div id="tr_down_cnsd">
						...
						<!-- 2012.03.12 사안개요는 조회조건은 정부서, 로그인 세션 부서가 법무팀, 정부서가 회신인 상태일 경우 조회 가능 added by hanjihoon -->
							<tr id="tr_main_matr_cont">
								<th><spring:message code="las.page.field.contractmanager.consideration_inner_d.main_matr_cont" /></th>
								<td colspan="3">
						...
						<table id="tab_contents_sub_conts3_3" cellspacing="0" cellpadding="0" border="0" class="table-style01">
							<colgroup>
								<col width="15%" />
								<col width="9%" />
								<col width="18%" />
						...
						<c:when test="${command.top_role == 'RA02' && lomRq.mn_cnsd_dept == command.blngt_orgnz}"><!-- 정_검토부서 담당자 -->
							...
							<table id="tab_contents_sub_conts3_3" cellspacing="0" cellpadding="0" border="0" class="table-style01" style="display:;">
								...
								<c:otherwise>
									...
									<tr name="tr_attached_file" style="display:none;">
										<th rowspan="3">
											<spring:message code="las.page.field.contractmanager.consideration_inner_d.attached_file" />
											<img src="/images/las/ko/common/step_q.gif" alt="info" title="DOC, DOCX, GUL, PDF, OCX, XLS, XLSX, PPT, PPTX,&#13;JPG, HWP, TIF, RTF, ZIP, PTX, TML, TXT, LSX, EML, BMP, GIF, MSG, ALZ, DOT &#13;&#13;<spring:message code='las.msg.com.fileFilter'/>" />
											<br/><br/>
											<!-- Sungwoo added 2014-09-15 -->
											<%-- <ul style="text-align: right; padding-right: 20%">
												<li>* <spring:message code="clm.page.msg.manage.copyOfReq" /></li>
												<li><spring:message code="clm.page.msg.common.contract" /> : <input id="reqContChk" name="reqContChk" type="checkbox" onclick="javascript:reqContAtt();"></input></li>
												<li><spring:message code="clm.page.msg.common.attachment" /> : <input id="reqAttChk" name="reqAttChk" type="checkbox" onclick="javascript:reqAtt();"></input></li>
												<li><spring:message code="clm.page.msg.common.etc" /> : <input id="otherAttChk" name="otherAttChk" type="checkbox" onclick="javascript:otherAtt();"></input></li>
												<li><spring:message code="clm.page.msg.common.all" /> : <input id="allAttChk" name="allAttChk" type="checkbox" onclick="javascript:allReqAtt();"></input></li>
											</ul> --%>
										</th>
										<td><spring:message code="las.page.field.contractmanager.consideration_inner_d.contract" /></td><!-- 계약서 -->
										<td colspan="3">
				...
				<!-- inner_d_view 끝 -->
			<!--  필수 항목의 경우 최종본 일 경우에만 나타나게 된다. 김재원 -->
			<!-- Brice block 필수 항목  by request of Shron 2019-04-22-->	
			
				<c:if test="${'Y' eq lomRq.plndbn_req_yn && 'T0103' eq lomRq.biz_cd && 'T0201' eq lomRq.depth_cd && ('T030705' eq lomRq.cnsl_mid_cd3 || 'T030706' eq lomRq.cnsl_mid_cd3) }">
				<!-- 필수 항목 저장 내용 시작 김재원 -->
				 <!-- T&C 데이터이면서  Legal 계약이 없는 경우 최종본 승인때 필수조항 Checklist 버튼이 안보여야 함. -->
				<c:if test="${ ('T030705006' != lomRq.cntrt_trgt_cd 
				                                        && 'T030705003' != lomRq.cntrt_trgt_cd
				                                        && 'T030705002' != lomRq.cntrt_trgt_cd
				                                        && 'T030706006' != lomRq.cntrt_trgt_cd
				                                        && 'T030706003' != lomRq.cntrt_trgt_cd
				                                        && 'T030706002' != lomRq.cntrt_trgt_cd   )  }"> 
			<!-- 	<div id="tr_down100">
					<div class="title_basic">
					...
					<div class="textBox">
						<table>
							...
							<c:choose>
							    <c:when test="${empty lomMn.cnsd_status || 'C04302' eq lomMn.cnsd_status }">
							        <tr>
					...
					<!--// 필수 항목 체크 리스트 -->
			<!-- 	</div> -->
				</c:if><!-- T&C 데이터이면서  Legal 계약이 없는 경우 최종본 승인때 필수조항 Checklist 버튼이 안보여야 함. -->
			 </c:if>			
			...
			<!-- //이력정보 -->
			<!-- HQ 이력정보 START -->
				<c:if test="${'Y' eq lomRq.hq_req_yn }">
				<div class="title_basic">
					<h4>Suwon Office  <spring:message code="las.page.field.contractmanager.consideration_his.review_history" /><!-- 검토이력 --> <img src="<%=IMAGE%>/btn/btn_up.gif" alt="down" onclick="tit_Toggle(this,'hq_reviewHisView');" style='cursor;pointer'/></h4>
				</div>
							...
							<c:forEach var="list_in" items="${list.HQ_LIST}" varStatus="x">			
								<c:choose>
									<c:when test="${'REQ' eq list_in.cont_type }">	
										<c:set var="cont_lavel1"  value="Suwon Office REVIEW REQUEST"/>
									</c:when>
									<c:when test="${'ADMIN_REPLY' eq list_in.cont_type }">	
										<c:set var="cont_lavel1"  value="Suwon Office REVIEW REPLY"/>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test="${'RP010' eq list_in.hq_rel_pro }">
												<c:set var="cont_lavel1"  value="SO(CE) Legal Opnion"/>
												<c:set var="cont_lavel2"  value="SO(IM) Legal Opnion"/>	
											</c:when>
											<c:when test="${'RP020' eq list_in.hq_rel_pro }">
												<c:set var="cont_lavel1"  value="SO(IM) Legal Opnion"/>	
											</c:when>
											<c:when test="${'RP030' eq list_in.hq_rel_pro }">
												<c:set var="cont_lavel1"  value="Suwon Office(CE) Legal Opnion"/>
												<c:set var="cont_lavel2"  value="Suwon Office(IM) Legal Opnion"/>	
											</c:when>
											<c:otherwise>
												<c:set var="cont_lavel1"  value="Suwon Office Legal Opnion"/>						
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>				
								...
								<tr class="Nocol" id="tr_show_hq" style="display:none">
	 								<td class="nopadding" colspan="3" style='padding:6px 1px 6px 0'>
										<table class="table-style_sub02">
											...
											<c:choose>
												<c:when test="${ list_in.cont_type ne 'ADMIN_REPLY'  }">
													<tr class="Nocol">
												...
												<c:when test="${ list_in.cont_type eq 'ADMIN_REPLY'  }">											
													<tr class="Nocol">
														<th><spring:message code="las.page.field.contractmanager.consideration_his.attached_file" /><!-- 첨부파일 --></th>
														<td colspan="3">
															<iframe src="<c:url value='/clm/blank.do' />" id="fileList_<c:out value="${list_in.hq_cnsdreq_id}"  />_<c:out value="${list_in.cnsd_level}"  />" name="fileList_<c:out value="${list_in.hq_cnsdreq_id}"  />_<c:out value="${list_in.cnsd_level}"  />" frameborder="0" class='addfile_iframe_d' scrolling="no" allowTransparency="true" ></iframe>
					...
					<table id='tr_down01_resp' cellspacing='0' cellpadding='0' class='table-style01' border='0'>
						...
						<tr>
							<th>
								...
								<c:if test="${command.top_role == 'RA01' && (lomRq.depth_status != 'C02607' && lomRq.depth_status != 'C02603') && (lomRq.prgrs_status == 'C04202' || lomRq.prgrs_status == 'C04203' || lomRq.prgrs_status == 'C04204' || lomRq.prgrs_status == 'C04206'  || lomRq.prgrs_status == 'C04207' )}">
				...
				<!--
				<div class='title_basic'>
					<h4>SUWON Review <img src="<%=IMAGE%>/btn/btn_up.gif" alt="up" onclick="tit_Toggle(this,'tr_down01_resp');" style='cursor;pointer'/></h4>
				</div>
				<table id='tr_down01_respX' cellspacing='0' cellpadding='0' class='table-style01' border='0'>
					<colgroup>
						<col width='15%' />
						<col width='85%' />
					</colgroup>							
					<tr>
						...
						<td>
							<span id="HQ_ECMS_contract_status_DESC">&nbsp;</span>
						</td>
					</tr>			
					<tr>
						<th>
						...
						<td>
							<span id="HQ_ECMS_hq_reply">&nbsp;</span>
						</td>
					</tr>
				</table>
				<script type="text/javascript">
				    document.getElementById("HQ_ECMS_contract_status_DESC").innerText = HQ_ECMS_contract_status_DESC;
				    document.getElementById("HQ_ECMS_hq_reply").innerText = HQ_ECMS_hq_reply;
				</script>				
			...
			<div class="btn_wrap_c">
				<BR>
				<c:if test="${command.top_role == 'RA01' && (lomRq.depth_status != 'C02607' && lomRq.depth_status != 'C02603' && lomRq.depth_status != 'C02609') && (lomRq.prgrs_status == 'C04202' || lomRq.prgrs_status == 'C04203'  || lomRq.prgrs_status == 'C04204' )   && (empty lomResp)  }"><!-- 최초 배정전 상태 -->
					<span id="btn_down15"  class="btn_all_w" onclick="popReturn();"><span class="mail"></span><a><spring:message code="las.filed.considetration.adminreply"/></a></span><!-- Admin Reply -->
				</c:if>
				<c:if test="${command.top_role == 'RA01' && (lomRq.CLOSE_YN eq 'N' || empty lomRq.CLOSE_YN) }"><!-- CLOSE 되어있지 않은 상태 -->
					<span id="btn_up16"  class="btn_all_w" onclick="popCloseReq();"><span class="end"></span><a><spring:message code="clm.page.button.contract.close"/></a></span><!--Close-->
				</c:if>
				...
				<span id="btn_down3" class="btn_all_w" style="display:none" onclick="forwardConsideration('DISPATCH');">
					<span class="mail"></span>
					<c:choose>
						<c:when test="${(lomRq.plndbn_req_yn == 'Y')}">
							<a><spring:message code="las.page.field.contractManager.sendFinal"/></a>
				...
				<!-- HQ REQUEST 버튼 하단 -->
			    <span id="btn_down20" style="display:none"><span class="btn_all_w" onclick="forwardConsideration('HQ');" ><span class="mail"></span><a><spring:message code="las.page.field.hqrequest.page17"/></a></span></span><!-- REQUEST TO HQ -->
			    <span id="btn_down21" class="btn_all_w" style="display:none" onclick="forwardConsideration('REHQ');" ><span class="mail"></span><a><spring:message code="las.page.field.hqrequest.page18"/></a></span><!-- REQUEST TO HQ 재의뢰-->
