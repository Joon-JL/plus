...
<%--
/**
 * 파  일  명 : ConsiderationHQ_l.jsp
 * 프로그램명 : 의뢰내역 - 본사검토의뢰 목록
 * 설      명 : 
 * 작  성  자 : 박병주
 * 작  성  일 : 2014.04.28
 */
...
$(document).ready(function(){
	...
	// HQ 검토상태
	getCodeSelectByUpCd("frm", "srch_prgrs_status_hq", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=LAS&combo_abbr=A&combo_grp_cd=C162&combo_type=S&combo_del_yn=N&combo_selected='+'<c:out value='${command.srch_prgrs_status_hq}'/>');
	...
	getCodeSelectByUpCd("frm", "sElComp", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=LAS&combo_abbr=A&combo_grp_cd=COMP2&combo_type=T&combo_del_yn=N&sel_grd=<c:out value='${command.sSel_grd}'/>&combo_selected='+'<c:out value='${command.sElComp}'/>');
...
$(function() {
	...
	/* HQ REVIEW 임직원 검색 */ 
	$("#srch_hq_reqman_nm").keydown(function(event){
		$("#srch_respman_id").val('');
		if(event.keyCode == "13") {
			popRespmanHQ();
			$("#srch_hq_reqman_nm").val('');
			return false;
		}
	});
	...
	*검토의뢰건 상세 보기 
	*/
	function goDetail(cnsdreq_id, cntrt_oppnt_nm, hq_cnsdreq_id , cnsd_level ){
		var frm = document.frm;	

		frm.cnsdreq_id.value = cnsdreq_id;				// 검토의뢰ID

		frm.hq_cnsdreq_id.value = hq_cnsdreq_id;
		frm.cnsd_level.value = cnsd_level;   // hq 검토 의뢰 차수
	...
	*/
	function popRespmanHQ() {
		var frm = document.frm;
    	var srchValue = comTrim($("#srch_hq_reqman_nm").val());
    	$("#srch_man_div").val("HQ_RESP");
		...
		frm.srch_user_cntnt.value = $("#srch_hq_reqman_nm").val();
	...
	*/
	function setUserInfos(obj) {
		...
		}else if($("#srch_man_div").val() == "HQ_RESP"){
			$("#srch_hq_reqman_nm").val(userInfo.cn);
			$("#srch_hq_respman_id").val(userInfo.epid);	
		}
	}
...
<div id="wrap">	
	...
	<div id="container">
		...
		<!-- title -->
		<div class="title">
			<h3><spring:message code="las.page.field.lawconsideration.reviewCont"/> HQ </h3><!-- HQ 계약검토 -->
		</div>
		<!-- //title -->
		...
		<div id="content">
			...
			<input type="hidden" name="srch_hq_respman_id" value="<c:out value='${command.srch_hq_respman_id}'/>" />
			...
			<input type="hidden" name="hq_cnsdreq_id" id="hq_cnsdreq_id"  value="" />	<!-- HQ 검토의뢰 아이디-->	
			<input type="hidden" name="cnsd_level" id="cnsd_level"  value="" />	<!-- HQ 검토 레벨 차수 -->	
						...
						<tr>
							<td>
								<table class="search_form">
									...
									<tr>
										<th>HQ <spring:message code="las.page.field.contractmanager.consideration.prgrs_status" /></th>
										 <td> 
											<select name="srch_prgrs_status_hq" id="srch_prgrs_status_hq" style="width:182px" />
											</select>
										</td>
										
										<th>
											HQ <spring:message code="las.page.field.manageCommon.srchLasCnsdmanNm" /><!-- HQ 검토자 -->
										</th>
										<td>
											<input type="text" name="srch_hq_reqman_nm" id="srch_hq_reqman_nm" value="<c:out value='${command.srch_hq_reqman_nm}'/>" maxLength="100" class="text"  style='width:179px'/><!-- HQ 검토자 -->
										</td>
										
										<!-- HQ01 권한자일 경우  콤보 리스트가 나타나게 된다. -->
										<c:choose>
			...
			<table class="list_basic">
				...
				<thead>
					<tr>
			            ...
			            <th colspan="3"><spring:message code="las.page.field.statistics.lap.subsidiary"/></th>	<!-- subsidiary -->
						<th colspan="2">HQ</th>	<!-- 본사 -->
					</tr>
					<tr>
			            <th class="sub01"><spring:message code="las.page.field.contractManager.reviewer"/></th>	<!-- 법무담당자 -->
			            <th class="sub01">Reviewer<br><spring:message code="las.page.field.contractManager.state"/></th><!-- 의뢰상태 -->				            
			            <th class="sub01"><spring:message code="las.page.field.contractManager.state"/></th><!-- 검토상태 -->	
			            <th class="sub01">HQ <spring:message code="las.page.field.contractManager.reviewer"/></th>	<!-- 법무담당자 -->
			            <th class="sub01">HQ <spring:message code="las.page.field.contractManager.state"/></th><!-- HQ 검토상태 -->				            
					</tr>
				</thead>
				<tbody>
						<c:choose>
...
<%-- 											중요도 체크 로직 주석 처리 	<c:choose>
						            		...
						            		<a href='javascript:goDetail("<c:out value='${list.cnsdreq_id}'/>", "<c:out value='${list.cntrt_oppnt_nm}'/>","<c:out value='${list.hq_cnsdreq_id}'/>" ,"<c:out value='${list.hq_cnsd_level}'/>" );' > 
						            ...
						            <td class="tC"><c:out value='${list.law_status_nm}' escapeXml='false' /></td>
							            <c:choose>
									...
									<c:out value='${list.prgrs_status_nm}' escapeXml='false' /></td>	
									 ...
									 <c:choose>
						            	<c:when test="${list.HQ_CNSD_STATUS_NM eq ''}">
						            		<td class="tC txtover" title="">&nbsp;</td>
						            	</c:when>
						            	<c:when test="${list.RESP_NUM_HQ > 1}">
						            		<td class="tC txtover" title="<c:out value='${list.HQ_RESPMAN_NM}'/>"><c:out value='${list.hq_respman_fnm}' escapeXml='false' /><br><spring:message code="las.page.field.contractManager.etc"/><c:out value='${list.RESP_NUM_HQ}' escapeXml='false' /><spring:message code="las.page.field.contractManager.nmC"/>
						            		</td>
						            	</c:when>
						            	<c:otherwise>
						            		<td class="tC txtover" title="<c:out value='${list.HQ_RESPMAN_NM}'/>"><c:out value='${list.hq_respman_fnm}' escapeXml='false' /></td>
						            	</c:otherwise>
						            </c:choose>		
					            	...
					            	<%-- 	<td class="tC txtover" title="<c:out value='${list.hq_respman_nm}'/>"><c:out value='${list.hq_respman_fnm}' escapeXml='false' /></td> --%>
							            ...
							            <c:choose>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16201'}"><td class="green_cor tC"></c:when>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16202'}"><td class="bora_cor tC"></c:when>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16203'}"><td class="blue_cor tC"></c:when>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16204'}"><td class="bora_cor tC"></c:when>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16205'}"><td class="yellow_cor tC"></c:when>
								            <c:when test="${list.HQ_CNSD_STATUS eq 'C16206'}"><td class="red_cor tC"></c:when>
											<c:otherwise><td class="tC"></c:otherwise>
										</c:choose>
									<c:out value='${list.HQ_CNSD_STATUS_NM}' escapeXml='false' /></td>	
						    ...
						    <c:otherwise>
								<tr onMouseOut="this.className='';" onMouseOver="this.className='selected'">								
									<c:choose>
											<c:when test="${elUserlYn}">
												<td colspan="11" align="center" class="tC"><spring:message code="las.msg.succ.noResult" /></td>
											</c:when>
											<c:otherwise>
												<td colspan="10" align="center" class="tC"><spring:message code="las.msg.succ.noResult" /></td>
											</c:otherwise>
										</c:choose>																	
								</tr>
						    </c:otherwise>
