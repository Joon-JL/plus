...
<%-- 
/**
 * 파  일  명 : ManageRegistrationApproval_l.jsp
 * 프로그램명 : 체결후등록 승인목록
 * 설      명 : 
 * 작  성  자 : 신남원
 * 작  성  일 : 2011.10.04
...
<script language="javascript">
	...
	$(document).ready(function(){
		...
		//신성우 변경처리 2014-04-10 Post Conclusion 검색조건으로 수정
...
<form:form name="frm" id='frm' method="post" autocomplete="off"> 
	...
	<input type="hidden" name="page_gbn"				id="page_gbn" value="registApproval" />
								...
								<table class="search_form" style='min-width:700px;'>
									...
									<tr>
									   <th><spring:message code="clm.page.field.manageCommon.srchReqmanNm" /></th>
                                        ...
                                        <td>
                                            <select class="srch" name="srch_cnclsnpurps_bigclsfcn" id="srch_cnclsnpurps_bigclsfcn" style="width:204px">
                                            </select>   
                                        </td>
                                         --%>
									</tr>
									...
									<tr>
									   <th><spring:message code="clm.page.field.manageCommon.srchRespDept" /></th>
                                        <td>
                                            <input class="text" style="width:184px" type="text" name="srch_resp_dept_nm" id="srch_resp_dept_nm" value="<c:out value='${command.srch_resp_dept_nm}'/>" onKeypress="if(event.keyCode == 13){popDept();return false;}"/><img src="<%=IMAGE%>/icon/ico_search.gif" class="cp" alt="search" onclick="popDept()" style="cursor:pointer;margin-left:-1px" />
                                        </td>
                                        <th><spring:message code="clm.page.field.manageCommon.srchBizclsfcn" /></th>
                                        <td>
                                            <select class="srch" name="srch_biz_clsfcn" id="srch_biz_clsfcn" style="width:204px">
                                            </select>   
                                        </td>
                                        <th><spring:message code="clm.page.field.manageCommon.srchCnclsnpurpsBigclsfcn" /></th>
                                        <td>
                                     ...
                                     <tr>   
									   <th><spring:message code="clm.page.field.manageCommon.srchStep" /> / <spring:message code="clm.page.field.manageCommon.srchStatus" /></th>
                                        <td>
                                            ...
                                            / --%>     
                                            <spring:message code="las.page.title.main.consultation"/>&nbsp;&nbsp;/&nbsp;&nbsp;      
                                        ...
                                        <td>
                                            <select class="srch" name="srch_if_sys_cd"  id="srch_if_sys_cd" style="width:204px" >
                                            </select>
                                        </td>
                                         --%>
                                         <td></td>
			  ...
			  <%
				...
				%>			
				    <tr  <%if(idx == resultList.size()-1){%>class="end"<%}%> onMouseOut="this.className='<%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onMouseOver="this.className='on <%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onClick="javascript:detailAction('<%=lom.get("cnsdreq_id") %>', '<%=lom.get("prcs_depth")%>','<%=lom.get("depth_status")%>');">
				      ...
				      <td class="tL txtover" title="<%=lom.get("cnsdmans") %>"><nobr><%=lom.get("cnsdman") %></nobr></td>
				      <td><nobr><%=DateUtil.formatDate((String)lom.get("cntrt_cnclsnday"),"-") %></nobr></td>
				      <td class="tL txtover" title="<%=lom.get("prcs_depth_nm") %>"><nobr><%=lom.get("prcs_depth_nm") %></nobr></td>
				      <td class="tL txtover" title="<%=lom.get("depth_status_nm") %>"><nobr><%=lom.get("depth_status_nm") %></nobr></td>
			        </tr>
		      <%	}
				}else{
 			  %>
					<tr class="end">
					  <td colspan="8" class="tC"><spring:message code="secfw.msg.succ.noResult" /></td>
					</tr>
			  <%}%>			
			  </tbody>
			</table>
