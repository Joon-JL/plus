...
<% 
    ...
    // 2014-07-17 Kevin added.
    String basicInfoView = (String)lomRq.get("basicInfoView");
...
var chk_flag = false;
    ...
    $(document).ready(function(){
    	...
    	$("#collapse").click(function(){
    		$("#collapseOne").fadeToggle("slow", function(){toggleTextChange();});
    		$("#cbToggle").attr("checked", false);
    	});
    	
    	// initally hide basic information section.
    	<% if(basicInfoView.equals("")){%>			// default hide mode
    		$("#collapse").click();
    	<%} else if(basicInfoView.equals("h")){%>	// hide mode by user decision
    		$("#collapse").click();
    		$("#cbToggle").attr("checked", true);
    	<%} else if(basicInfoView.equals("s")){%>	// show mode by user decision
    		$("#cbToggle").attr("checked", true);
    	<%}%>
    	
    	// always show/hide toggle checkbox
    	$("#cbToggle").click(function(){
    		// s: to be shown
    		// h: to be hidden
    		// n: none
    		var isShown = $("#collapseOne").is(":visible");
    		if($(this).is(":checked")){
    			$("#toggleConfigVal").val((isShown ? "s" : "h"));
    		} else {
    			$("#toggleConfigVal").val("n");
    		}
    	});
    });
    ...
    function toggleTextChange(){
    	if($("#collapseOne").is(":visible")){
    		$("#toggleSign").text("- Hide");
    		$("#saveToggle").text("Always show");
    	} else{
    		$("#toggleSign").text("+ Show");
    		$("#saveToggle").text("Always hide");
    	}
    }
...
<div id="wrap">
	...
	<div id="container">
	   ...
	   <input type="hidden" name="cntrt_untprc_chk" id="cntrt_untprc_chk" value="<c:out value='${lomRq.cntrt_untprc_expl}'/>" />
	    <input type="hidden" name="toggleConfigVal" id="toggleConfigVal" />
	   <!-- cntrt_id 배열 표시  -->
	   ...
	   <input type="hidden" name="orgnz_nm_abbr" id="orgnz_nm_abbr" value="<c:out value='${command.abbr_comp_nm}'/>" />
		...
		<div id="content">
			...
			<table cellspacing="0" cellpadding="0" border="0" class="table-style01">
				...
				<tr>	<!--  Requst Title -->
					<th><spring:message code="clm.page.msg.manage.reqName" /><span class='astro'>*</span></th>
				  	<td colspan="6"><input type="text" name="req_title" name="id" alt="<spring:message code="clm.page.msg.manage.reqName" htmlEscape="true" />" required value="<c:out value="${lomRq.req_title}"  escapeXml="false"/>" class="text_full" maxlength="66"/></td>
				</tr>
				<tr>	<!-- Purpose, Background and Request Details -->
					<c:choose>
					...
					<td colspan="6">
	                    ...
	                    <textarea name="body_mime" id="body_mime" cols="100%" rows="15" maxLength="4000" onkeyup="frmChkLenLang(this,4000,'curByteExpl_body_mine','<%=langCd%>')"  required class="text_area_full"><c:out value="${lomRq.cnsd_demnd_cont}" escapeXml="" /> </textarea>
					</td>
				</tr>
				<tr>	<!-- Counterparty , Contract Period , Payment/Collection -->
				...
				<tr>	<!-- Contract Amount , Currency Unit -->
	                 <th><spring:message code="clm.page.msg.manage.contAmt" /></th>
                    <td colspan="4">
                        <input type="text" name="cntrt_amt" id="cntrt_amt" value="<c:out value='${lomRq.cntrt_amt}' />" onclick="javascript:paymentGbnChange2();" onkeyup="olnyNum(this)";  style="width: 150px; text-align: right; margin-right: 1px;" maxlength="19" />
				...
				<tr class="slide-target02 slide-area">	<!-- Attachment -->
			...
			<div class='title_basic3'>
				<spring:message code="clm.page.msg.manage.bscInf" />
				<a id="collapse" style="margin-left:10px; cursor:pointer;">
					<span id="toggleSign" class="label label-danger" style="font-size:11px;">- Hide</span>
				</a>
				<span style="margin-left:5px;">
					<input type="checkbox" id="cbToggle" name="cbToggle" style="margin-top:0px; margin-right:5px; border:0px;" /><span id="saveToggle" style="font-weight:normal">Always show</span>
				</span>
			</div>
			<div id="collapseOne">
				<!-- //title -->
				<table class="table-style01">
		            ...
		            <tr>	<!-- Requestor , Request Date ,  Requested Date For Reply -->
					...
					<tr class="lineAdd">	<!--  CC -->
						...
						<td colspan="6">
							<span class="btn_all_b" onclick="javascript:openChooseClient();">
								<span class="add"></span>
								<a> <spring:message code="clm.page.msg.manage.add" htmlEscape="true" /></a>
							</span>									
							<span id="id_trgtman_nm"></span>
						</td>
					</tr>			
					<tr>	<!-- Contract Type -->
	                    ...
	                    <td colspan="5">
	                        ...
	                        <input disabled type="text" name="biz_clsfcn_nm" id="biz_clsfcn_nm" required alt="<spring:message code="clm.page.msg.manage.contType" htmlEscape="true" />" value="<c:out value='${lomRq.biz_clsfcn_nm}'/>" style="width:30%" />
	                        <input disabled type="text" name="depth_clsfcn_nm" id="depth_clsfcn_nm" required alt="<spring:message code="clm.page.msg.manage.contType" htmlEscape="true" />" value="<c:out value='${lomRq.depth_clsfcn_nm}'/>" style="width:20%" />
	                        <input disabled type="text" name="cnclsnpurps_bigclsfcn_nm" id="cnclsnpurps_bigclsfcn_nm" required alt="<spring:message code="clm.page.msg.manage.contType" htmlEscape="true" />" value="<c:out value='${lomRq.cnclsnpurps_bigclsfcn_nm}'/>" style="width:20%" />
	                        <input disabled type="text" name="cnclsnpurps_midclsfcn_nm" id="cnclsnpurps_midclsfcn_nm" required alt="<spring:message code="clm.page.msg.manage.contType" htmlEscape="true" />" value="<c:out value='${lomRq.cnclsnpurps_midclsfcn_nm}'/>" style="width:20%" />
	                        <a href="javascript:re_openChooseContractType();"><img src="<%=IMAGE %>/icon/ico_search_g.gif" /></a>  
	                    </td>
		             </tr>
	                <tr> <!-- Precedents -->
						<th style='border-left:0'><spring:message code="clm.page.field.samplecontract.title" /></th>	
	                    <td class='tC' colspan='6'><a href="javascript:openSampleContract();"><img src="<%=IMAGE %>/icon/ico_search_g.gif" class="cp" alt="search" /></a></td>
					</tr>
	                <tr>	<!-- Other Comments -->
	                ...
	                <tr id="trSpecialAttr">	<!-- Special Request -->
	                ...
	                <%  for(int j =0; j<listTs.size(); j++){
	                        ...
	                        if(j%2 == 0){                                   %>                              
	                            <tr>
	                            <th><span><%=(String)lom.get("attr_nm")     %>
	                            </span></th>
	                <%          if(j == listTs.size()-1){                   %>
	                ...
	                <%          }                                           %>
	                                ...
	                                <input type="text" name="arr_attr_cont" <%=dpMndtry %> id="arr_attr_cont" value="<%=StringUtil.convertHtmlTochars(StringUtil.bvl((String)lom.get("attr_value"), ""))%>" maxlength="240" class="text_full" style="margin-top:4px;" /></td>
	                <%      }else if(j%2 == 1){                             %>
