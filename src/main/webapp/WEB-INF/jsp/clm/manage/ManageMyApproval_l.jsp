...
<%-- 
/**
 * 파  일  명 : ManageMyApproval_l.jsp
 * 프로그램명 : MyApproval 목록(결재)
 * 설      명 : 
 * 작  성  자 : 김재원
 * 작  성  일 : 2013.04
 */
...
<html xmlns="http://www.w3.org/1999/xhtml"  lang="ko" xml:lang="ko"><!-- ManageMyApproval_l.jsp -->
<head>
...
<script language="javascript">
	...
	$(document).ready(function(){
		...
		원본접수의 단계/상태는 계약등록만 선택이 되어야한다
		계약정보수정/원본출납관리의 단계/상태는 계약등록,체결후등록,계약이행,종료,사별이관만 선택이 되어야한다
		한화면을 원본접수와 계약정보수정 화면이 같이 쓰기 때문에 IF문으로 분기. 
		*/
		
		var select_cd = "";
		
		if("<c:out value='${command.menu_id}'/>" == '20130319155330501_0000000362'){//원본접수
			select_cd ="C02501,C02502,C02508,C02504,C02505,C02507";
		}else if("<c:out value='${command.menu_id}'/>" == '20130319155330823_0000000366'){//계약정보수정/원본출납관리
			select_cd="C02501,C02502";
			getCodeSelectByUpCd("frm", "srch_step", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=LAS&combo_abbr=F&combo_grp_cd=C025&combo_type=T&combo_del_yn=N&select_cd='+select_cd+'&combo_selected=<c:out value='${command.srch_step}'/>');
...
<div id="wrap">
	...
	<div id="container">
		...
		<div class="title">
			<c:choose>
				<c:when test="${command.isOrgMgr == 'Y' }">
					<h3><spring:message code="clm.page.msg.manage.contRevision" /></h3>
				</c:when>
				<c:otherwise>
					<h3><spring:message code="clm.page.title.manageMyApproval.listTitle" /></h3>
				</c:otherwise>
			</c:choose>
		...
		<div id="content">
			...
			<form:form name="frm" id='frm' method="post" autocomplete="off"> 
				...
				<input type="hidden" name="isOrgMgr" id="isOrgMgr" value="<c:out value='${command.isOrgMgr}'/>"/>
				
				<!-- 단계/상태(원본접수에는 계약등록/원본미접수 상태만) -->
				<!--input type="hidden" name="srch_step" value="C02503" /-->
									...
									<tr>
                                     ...
                                     <tr>   
									   <th><spring:message code="clm.page.field.manageCommon.srchStep" /> / <spring:message code="clm.page.field.manageCommon.srchStatus" /></th>
                                        ...
                                        <c:choose>
                                        	<c:when test="${command.menu_id eq '20130319155330501_0000000362'}">
                                        		<div class='icon_stepWrap'>
													<div class='icon_step'><spring:message code="clm.page.msg.manage.regCont" /></div>
													<div class='icon_status'><spring:message code="clm.page.title.manageMyApproval.notreceived" text="원본미접수"/></div>
												</div>
                                        	</c:when>
                                        	<c:when test="${command.menu_id eq '20130319155330823_0000000366'}">
										...
										<td class="vb tR">
											<a href="javascript:searchAction('');"><img src="<%=IMAGE%>/btn/btn_search.gif" alt="<spring:message code="clm.page.msg.common.inquire" htmlEscape="true" />" class='mt5' style='margin-right:9%'/></a>
										</td>									
									</tr>
								</table>
			  	...
			  	<thead>
					<tr>
						...
						<th class='overflow' title='<spring:message code="las.page.field.contractmanager.consideration.first"/> <spring:message code="clm.page.field.manageCommon.reqDay" />&#13;(<spring:message code="clm.page.field.contract.conclusion.detail.conclusionorgacceptday" />)'>
						    ...
						    <div class='overflow tC' style='width:100%;'><spring:message code="las.page.field.contractmanager.consideration.first"/> <spring:message code="clm.page.field.manageCommon.reqDay" /></div>
						    <div class='overflow tC' style='width:100%;'>(<spring:message code="clm.page.field.contract.conclusion.detail.conclusionorgacceptday" />)</div>
			  ...
			  <%
				...
				%>			
				    <tr  <%if(idx == resultList.size()-1){%>class="end"<%}%> onmouseout="this.className='<%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onMouseOver="this.className='on <%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onClick="javascript:detailAction('<%=lom.get("cnsdreq_id") %>', '<%=lom.get("prcs_depth")%>','<%=lom.get("depth_status")%>');">
				      ...
				      <td class="txtover tC" title="<%=StringUtil.replace((String)lom.get("cnsdman"),"<br>","&#13;") %>"><%=lom.get("cnsdman") %></td>
