...
<%-- 
/**
 * 파  일  명 : RegManageRegistration_l_new.jsp
 * 프로그램명 : 체결후등록 목록
 * 설      명 : 
 * 작  성  자 : 김재원
 * 작  성  일 : 2013.05
...
<html><!-- RegManageRegistration_l_new.jsp -->
<head>
<meta charset="utf-8" />
...
// 		getCodeSelectByUpCd("frm", "srch_step", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=LAS&combo_abbr=F&combo_grp_cd=C025&select_cd=C02501&combo_type=T&combo_del_yn=N&combo_selected='+'<c:out value='${command.srch_step}'/>');
		getCodeSelectByUpCd("frm", "srch_state", '/common/clmsCom.do?method=listComCdByGrpCd&combo_sys_cd=LAS&combo_abbr=F&combo_grp_cd=C02508&combo_type=T&combo_del_yn=N&combo_selected='+'<c:out value='${command.srch_state}'/>');
		//비즈니스 단계 
...
//		alert(display);
	...
	*/ 
	...
	<%}%>
		...
		frm.target = "_self";
		frm.action = "<c:url value='/clm/manage/registration.do' />";
	    frm.method.value = "listRegistrationExcel";
	    frm.curPage.value = "1";
	    frm.submit();
	}
	...
	* 관리페이지 이동
	*/	
	function detailAction(id, prcs_depth, depth_status){
		var frm = document.frm;
		
		viewHiddenProgress(true) ;
		frm.target = "_self";
		frm.action = "<c:url value='/clm/manage/registration.do' />";
		frm.method.value = "detailRegistration";
	...
	*/
	function goInsertForm() {		
		...
		frm.target = "_self" ;
		frm.action = "<c:url value='/clm/manage/registration.do' />";
		frm.method.value = "forwardInsertRegistration" ;
	...
	<div id="container">
		...
		<div id="content">
			...
			<form:form name="frm" id='frm' method="post" autocomplete="off"> 
				...
				<input type="hidden" name="pageGbn"/>
						...
						<tr>
							<td>
								<table class="search_form" style='min-width:700px;'>
									...
									<tr>
										<th><spring:message code="clm.page.field.manageCommon.srchReqTitle" /><!-- 의뢰명 --></th>
										<td>
...
<%--                                         <th><spring:message code="clm.page.field.manageCommon.srchRespmanNm" /><!-- 담당자 --></th> --%>
<!--                                         <td> -->
...
<%--                                         <th><spring:message code="clm.page.field.manageCommon.srchCnsdmanNm" /><!-- 검토자 --></th> --%>
<!--                                         <td> -->
<%--                                             <input class="text" disabled style="width:200px" type="text" name="srch_cnsdman_nm" id="srch_cnsdman_nm" value="<c:out value='${command.srch_cnsdman_nm}'/>" /> --%>
<!--                                         </td> -->
                                        <th><spring:message code="clm.page.field.manageCommon.srchOppntCd" /><!-- 계약상대방 --></th>
										<td>
                                        ...
                                        <th>
                                        <!-- !@# 현재 목록에서의 체결후등록 계약 단계는 모두 체결전이므로 단계 생략 -->
									   		<spring:message code="clm.page.field.manageCommon.srchStep" /><!-- 단계 --> / <spring:message code="clm.page.field.manageCommon.srchStatus" /><!-- 상태 -->
									   	</th>
                                        <td>
<%--                                             <select class="srch" name="srch_step" id="srch_step" value="<c:out value='${command.srch_step}'/>" style='width:95px'> --%>
<!--                                             </select> -->
<!--                                             /            -->
											<spring:message code="las.page.title.main.consultation"/>&nbsp;&nbsp;/&nbsp;&nbsp;<select class="srch" name="srch_state"  id="srch_state" style='width:85px'>
                                            </select>
                                        </td>     
                                        
<!-- 									</tr> -->
<!-- 									<tr> -->
<%--                                         <th><spring:message code="clm.page.field.manageCommon.srchBizclsfcn" /><!-- 비즈니스단계 --></th> --%>
<!--                                         <td> -->
<!--                                             <select class="srch" name="srch_biz_clsfcn" id="srch_biz_clsfcn" style="width:200px"> -->
<!--                                             </select>    -->
<!--                                         </td> -->
<%--                                         <th><spring:message code="clm.page.field.manageCommon.srchCnclsnpurpsBigclsfcn" /><!-- 체결목적 --></th> --%>
<!--                                         <td> -->
...
<!--                                         </td> -->
									...
									<tr>
									   <th><spring:message code="clm.page.field.manageCommon.srchReqDay" /><!-- 의뢰일 --></th>
                                        <td>
...
<%--                                        <th><spring:message code="clm.page.field.manageCommon.srchCnlsnday" /><!-- 체결일 --></th> --%>
<!--                                         <td> -->
...
<!--                                         </td> -->
                                        ...
                                        <th><spring:message code="clm.page.field.contract.conclusion.detail.conclusionorgacceptday" /><!-- 원본접수일 --></th>
                                        <td>
                                    	...
                                    	<td></td>
                                    	<td class="tR">
											<a href="javascript:searchAction('');"><img src="<%=IMAGE%>/btn/btn_search.gif" alt="<spring:message code="clm.page.msg.common.search" htmlEscape="true" />" style='margin-right:9%' class='mt5'/></a>
										</td>
                                    </tr>
			...
			<div class="t_titBtn">
				<div class="btn_wrap_t">
					<span class="btn_all_w" onclick="goInsertForm();"><span class="edit"></span><a><spring:message code="clm.page.msg.manage.aftConclWrite" /></a></span>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="list_basic">
				...
				<colgroup>
			  	...
			  	<thead>
					<tr>
						<th><spring:message code="clm.page.field.manageCommon.reqTitle" /><!-- 의뢰명 --><br>(<spring:message code="clm.page.field.manageCommon.srchCntrtNm" /><!-- 계약명 -->)</th>
				...
				%>			
				    <tr  <%if(idx == resultList.size()-1){%>class="end"<%}%> onMouseOut="this.className='<%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onMouseOver="this.className='on <%if(idx == resultList.size()-1){%>end<%}%>';this.style.cursor='pointer'" onClick="javascript:detailAction('<%=lom.get("cnsdreq_id") %>', '<%=lom.get("prcs_depth")%>','<%=lom.get("depth_status")%>');">
				      ...
				      <td class="txtover" title="<%=StringUtil.bvl(lom.get("cntrt_no"),"") %>"><%=StringUtil.bvl(lom.get("cntrt_no"),"") %></td>
		      ...
		      <%	}
 			  ...
 			  %>
					<tr>
					 <td colspan="8" class="tC"><spring:message code="las.msg.succ.noResult" /></td>
					</tr>
			  <%}%>		
			  </tbody>
			</table>
