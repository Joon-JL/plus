...
<%
	/// Fernando 08-12-2023 (start) ///
	boolean l_isAdmin = false;
	boolean l_isReviewer = false;
	...
	boolean l_isAdminOrReviewer = l_isAdmin || l_isReviewer;
	/// Fernando 08-12-2023 (end) ///
	
	/// Fernando 16-07-2024 (start) ///
...
<script language="javascript">
	...
	$(function() {
		...
		//var l_today_val = l_today.toLocaleDateString("fr-CA");
		var mm1 = ("0" + (l_today.getMonth() + 1)).slice(-2);
		var dd1 =("0" + (l_today.getDate())).slice(-2);
		var yy1 = l_today.getFullYear();
		var l_today_val = "" + yy1 + '-' + mm1 + '-' + dd1; 
		
		var l_tomorrow = new Date( 0 + Math.abs(l_today) + (1000 * 60 * 60 * 24/*1 day*/) );
		//var l_tomorrow_val = l_tomorrow.toLocaleDateString("fr-CA");
		var mm2 = ("0" + (l_tomorrow.getMonth() + 1)).slice(-2);
		var dd2 =("0" + (l_tomorrow.getDate())).slice(-2);
		var yy2 = l_tomorrow.getFullYear();
		var l_tomorrow_val = "" + yy2 + '-' + mm2 + '-' + dd2; 
		
		var l_srch_end_reqday_val = "" + $("#srch_end_reqday").val();
		var l_srch_end_reqday_tmp = l_srch_end_reqday_val.split("-");
		var l_srch_end_reqday     = new Date(l_srch_end_reqday_tmp[0], l_srch_end_reqday_tmp[1]-1, l_srch_end_reqday_tmp[2]);
		
		/*alert(l_today);
		alert(l_today_val);
		alert(l_tomorrow);
		alert(l_tomorrow_val);
		alert(l_srch_end_reqday);
		alert(l_srch_end_reqday_val);*/
		
		if ( (l_srch_end_reqday_val !== l_today_val) &&
			 (l_srch_end_reqday_val !== l_tomorrow_val) )
		{
			var l_new_srch_end_reqday = new Date( 0 + Math.abs(l_srch_end_reqday) + (1000 * 60 * 60 * 24/*1 day*/) );
			var mm3 = ("0" + (l_new_srch_end_reqday.getMonth() + 1)).slice(-2);
			var dd3 =("0" + (l_new_srch_end_reqday.getDate())).slice(-2);
			var yy3 = l_new_srch_end_reqday.getFullYear();
			var l_new_srch_end_reqday_val = yy3 + '-' + mm3 + '-' + dd3;
			
			/*alert(l_new_srch_end_reqday);
			alert(l_new_srch_end_reqday_val);*/
			
			$("#srch_end_reqday").val(l_new_srch_end_reqday_val);
		}
...
//		alert(display);
	...
	*/ 
	function excelDownLoad(){
		...
		var l_isPartner = "<%=l_isPartner%>".toUpperCase();
		if (l_isPartner === "TRUE") {
			alert('You have no access to download data.');
			return;
		}
		
		var agreed = confirm('This list contains personal information. Please refrain from sending it outside of Samsung, and exercise caution in its management.\n\nDo you agree?');
		if (!agreed) {
			return;
		}
		
		/// Fernando 19-12-2023 (start) ///
...
<script language="javascript">
	...
	{
		var l_user_AdminOrReviewer = <%=l_isAdminOrReviewer%>;
		
		if (l_user_AdminOrReviewer) {
			return true;
		}
		else {
...
<html>
<body oncopy="return <%=l_isAdminOrReviewer%>;" oncut="return <%=l_isAdminOrReviewer%>;" oncontextmenu='return blockRightClickfunction();' onload='showMessage("<c:out value='${returnMessage}'/>");'>
