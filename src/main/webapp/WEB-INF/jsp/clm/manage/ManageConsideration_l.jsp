...
<%-- 
/**
 * 파  일  명 : ManageConsideration_l.jsp
 * 프로그램명 : 계약검토목록
 * 설      명 : 
 * 작  성  자 : 김재원
 * 작  성  일 : 2013.04
...
<%
	...
	//연계시스템에서 넘어왔을경우 MTEC을 제외한 아래 연계시스템에서 넘어왔을경우 신규생성버튼을 없앤다.
	//BPOS, CMS, CPCEX, EMP, GEHS, IRP, myProject, PUWP, SSEMS
...
//		alert(display);
	...
	*/
	...
	function ConsiderationGuidePop(){
		...
		frm.forward_url.value="/step01<%=langCd.equals("ko")?"":"_"+langCd %>.html";
		frm.submit();		
	}
	
	function setInitControl() {
		//연계시스템에서 넘어온 경우 신규생성버튼을 막는다.
	<%
		if("BPOS".equals(infsysnm) || "CMS".equals(infsysnm) || "CPCEX".equals(infsysnm) || "EMP".equals(infsysnm) || "GEHS".equals(infsysnm) || "IRP".equals(infsysnm) || "myProject".equals(infsysnm) || "PUWP".equals(infsysnm) || "SSEMS".equals(infsysnm)){
	%>
		$('#btn_consider').attr("style","display:none");
	<%
		}
	%>
	}
...
<body onload='showMessage("<c:out value='${returnMessage}'/>","<c:out value='${command.submit_status}'/>");'> 
...
<div id="wrap">
	...
	<div id="container">
		...
		<!-- Title -->
		<div class="title">
			<h3><spring:message code="clm.page.title.manageConsideration.listTitle" /></h3>
		</div>
		<!-- //title -->			
		...
		<div id="content">
			<!-- tab -->
		  	<div class="tab_box">
		        ...
		        <span style="float:right;"><!-- <a href="javascript:ConsiderationGuidePop();"> --><a href="javascript:open_window('win', '../../step01<%=langCd.equals("ko")?"":"_"+langCd %>.html', 0, 0, 696, 588, 0, 0, 0, 0, 0)"><img src="<%=IMAGE%>/btn/btn_quick.gif" alt="Quick Guide"/></a></span>
			</div>
			<!-- //tab -->
			...
			<div class="t_titBtn">
				<div class="btn_wrap_t">
					<span class="btn_all_w" id="btn_consider"><span class="reply"></span><a href="javascript:goInsertForm();"><spring:message code="clm.page.msg.manage.createRevReq" /></a></span>
