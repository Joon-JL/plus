...
%>
    ...
    <div class="login_slas">
        <span class="logo"><img src="<%=IMAGE%>/common/login_logo.png" alt="logo"  /></span>
        <div class='user'>
        	<span><%=StringUtil.bvl((String)session.getAttribute("secfw.session.dept_nm_en"), "")%> &nbsp;</span>
	        <span><c:out value="${main_user_nm}"/></span>
 		</div>
        <div class='info_data'>
			<dl>
		...
		<a href="javascript:doAction();" class="btn_main">Confirm <img src="<%=IMAGE%>/icon/ico_arrow_yellow.png" /></a>
    </div>
    <!-- //login -->
<%
    if (spotViewFlag) {
%>
		...
		%>
			<a href='#' onclick="migUser();">.</a>
			<input type=hidden name='ecms_tran_yn' value='Y'>
			<script>
    ...
    <c:choose>
		<c:when test="${server_division=='DEV'}">
			...
			<div id="sessionConfig" style="display:none">
		      <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[SEUK] ============================================================</div>
		      <div><input type="radio" name="userid" id="userid" value="1" />&nbsp;Katie hughes(Requestor)</div>
		      <div><input type="radio" name="userid" id="userid" value="2" />&nbsp;Natalie Focosi(Admin)</div>
		      <div><input type="radio" name="userid" id="userid" value="3" />&nbsp;Phillip Carnell(Lawyer)</div>
		      <div><br/></div>
		      
		      <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[EHQ] ============================================================</div>
		      <div><input type="radio" name="userid" id="userid" value="20" />&nbsp;Siobhan Langmead(Requestor)</div>
		      <div><input type="radio" name="userid" id="userid" value="21" />&nbsp;Sharon Elizabeth Robinson(Admin)</div>
		      <div><input type="radio" name="userid" id="userid" value="22" />&nbsp;Edward Phillips(Lawyer)</div>
		      <div><br/></div>
		      
		      <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[SEF] =============================================================</div>
		      <div><input type="radio" name="userid" id="userid" value="30" />&nbsp;Claire Thiriez(Requestor)</div>
		      <div><input type="radio" name="userid" id="userid" value="31" />&nbsp;Bernard Vaudin(Admin)</div>
		      <div><input type="radio" name="userid" id="userid" value="32" />&nbsp;William Baffard(Lawyer)</div>
		      <div><br/></div>
		      
		      <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[SEG] =============================================================</div>
		      <div><input type="radio" name="userid" id="userid" value="40" />&nbsp;Chantal Privat(Requestor)</div>
		      <div><input type="radio" name="userid" id="userid" value="41" />&nbsp;Diane Kunz(Admin)</div>
		      <div><input type="radio" name="userid" id="userid" value="42" />&nbsp;STEFAN LAUN(Lawyer)</div>
		      <div><br/></div>
		      
		      <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[HQ] ==============================================================</div>
		      <div><input type="radio" name="userid" id="userid" value="100" />&nbsp;CE:최봉석 (HQ CE Leagal Head)</div>
		      <div><input type="radio" name="userid" id="userid" value="104" />&nbsp;IM:홍상범 (HQ IM Leagal Head)</div>
		      <div><input type="radio" name="userid" id="userid" value="102" />&nbsp;CE:최봉석 (HQ CE Reviewer)</div>
		      <div><input type="radio" name="userid" id="userid" value="103" />&nbsp;CE:양승훈 (HQ CE Reviewer)</div>
		      <div><input type="radio" name="userid" id="userid" value="101" />&nbsp;IM:한장수 (HQ IM Reviewer)</div>
		      <div><input type="radio" name="userid" id="userid" value="105" />&nbsp;IM:강은하 (HQ IM Reviewer)</div>
		      <div><input type="radio" name="userid" id="userid" value="106" />&nbsp;IM:이재준 (HQ IM Reviewer)</div>
		      <div><br/></div>
		</c:when>
...
<!-- //**************************** Form Setting **************************** -->
	...
	<div style="position: relative; margin-left: auto; margin-right: auto; top: 15px; border: 3px solid red; font-size: 150%; padding: 3px;">
		In accordance with our ABC policies relevant agreements must include wording to reflect our compliance commitments
	</div>
