...
<%
	...
	String[] sArray1 = null;
	String srch_bigclsfcn = "  ";
	if(statisticsForm.getSrch_bigclsfcn() != null && statisticsForm.getSrch_bigclsfcn().length() > 0){
		sArray1 = statisticsForm.getSrch_bigclsfcn().split(","); 
		for( int i = 0; i < sArray1.length; i++ ){
			if(i < sArray1.length -1){
				srch_bigclsfcn = srch_bigclsfcn + "'"+sArray1[i]+"',";
			}else{
				srch_bigclsfcn = srch_bigclsfcn + "'"+sArray1[i]+"' ";
			}
			
		}		
	}else if(statisticsForm.getCntrtType() != null && statisticsForm.getCntrtType().length()>0){
		srch_bigclsfcn = "'"+statisticsForm.getCntrtType()+"'";		
	}else{
		srch_bigclsfcn = "'T0301', 'T0302', 'T0303', 'T0304', 'T0305', 'T0306', 'T0307', 'T0308', 'T0309', 'T0310', 'T0311', 'T0312', 'T0313'";
	}
	//out.println(srch_bigclsfcn);
%>

<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><spring:message code="las.page.title.report.StatisticsContractByTrend1" /></title>
...
<!--
	$(document).ready(function(){
	    ...
	    //getCodeSelectByUpCd("frm", "srch_bigclsfcn", "/common/clmsCom.do?method=getComboHTML&combo_gbn=CONTRACTTYPE&combo_up_cd=<c:out value='${command.cntrtType}'/>&combo_abbr=F&combo_type=S&combo_del_yn=N&combo_selected=<c:out value='${command.srch_bigclsfcn}'/>");
...
<%
		int type=2;
		String mrd = new String(C.process(report_url+"trendByMonthT03.mrd",type)); 
		String dept = "";
		String mrd_param = "";
		String param = "";
		String srch_dt1 = "";
		...
		if(statisticsForm.getCntrtType() != null && statisticsForm.getCntrtType().equals("T01")){
			mrd = new String(C.process(report_url+"trendByMonthT01.mrd",type));
		}else if(statisticsForm.getCntrtType() != null && statisticsForm.getCntrtType().equals("T02")){
			mrd = new String(C.process(report_url+"trendByMonthT02.mrd",type));
		}
...
<div id="wrap">
	...
	<form:form id="frm" name="frm" method="post" autocomplete="off">
		...
		<!-- Title -->
		<div class="title">
		<h3><spring:message code="las.page.title.report.StatisticsContractByTrend1" /></h3>
		</div>
							...
							<table class="search_form">
								...
								<tr>
								    ...
								    <select name="cntrtType" id="cntrtType"  style='width:80'></select>
								      ...
								      <!-- 
		    	        		      <select name="cntrtType" id="cntrtType"  onChange='javascript:changeSubCd("srch_bigclsfcn", "CONTRACTTYPE", this.value);' style='width:50'></select>
									  <select multiple  name="srch_bigclsfcn" id="srch_bigclsfcn"  style='width:50'></select>
									  
									  <select name="srch_midclsfcn" id="srch_midclsfcn" onChange='javascript:changeSubCd("srch_smlclsfcn", "CONTRACTTYPE", this.value);' style='width:80'>
