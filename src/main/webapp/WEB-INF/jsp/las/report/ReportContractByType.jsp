...
<%--
/**
 * 파     일     명 : ReportContractByType.jsp
 * 프로그램명 : 총계약건수 by 계약유형
 * 설               명 : 
 * 작     성     자 : 서백진
 * 작     성     일 : 2011.09
...
<%
	...
	if(userRoleList != null && userRoleList.size() > 0){
	    if(userRoleList.contains("RA00") || userRoleList.contains("RA01") || userRoleList.contains("RA02")){ // RA00: 시스템관리자  RA01: 법무관리자
	        accessLevel = "A";
	    }else if(userRoleList.contains("RD01") || userRoleList.contains("RD02")){// RD01: 사업부계약관리자  RD02: 사업부계약담당자
	        accessLevel = "D";
	        blngt_orgnz_gb = "B";
	    }else if(userRoleList.contains("RC01")){// RC01: CP관리자
	        accessLevel = "B";
	    }else{
	        accessLevel = "C";
	    }
	...
	if(statisticsForm!=null){			
		...
		//report_url = "http://las.samsung.net/report/";
	}
...
<!--
	$(document).ready(function(){
	    ...
	    if("<c:out value='${command.cntrtType}'/>" == ""){
	    	getCodeSelectByUpCd("frm", "cntrtType", "/common/clmsCom.do?method=getComboHTML&combo_gbn=CONTRACTTYPE&combo_up_cd=TOP&combo_abbr=F&combo_type=S&combo_del_yn=N&combo_selected=T03");
	    }else{
	    	getCodeSelectByUpCd("frm", "cntrtType", "/common/clmsCom.do?method=getComboHTML&combo_gbn=CONTRACTTYPE&combo_up_cd=TOP&combo_abbr=F&combo_type=S&combo_del_yn=N&combo_selected=<c:out value='${command.cntrtType}'/>");
	    }
...
<%
		int type=2;
		String mrd = new String(C.process(report_url+"contractBytypeT03.mrd",type)); 
		String dept = "";
		String mrd_param = "";
		String param = "";
		String srch_dt1 = "";
		...
		if(statisticsForm.getCntrtType() != null && statisticsForm.getCntrtType().equals("T01")){
			mrd = new String(C.process(report_url+"contractBytypeT01.mrd",type));
		}else if(statisticsForm.getCntrtType() != null && statisticsForm.getCntrtType().equals("T02")){
			mrd = new String(C.process(report_url+"contractBytypeT02.mrd",type));
		}else{
			if(statisticsForm.getStart_index() != null && statisticsForm.getStart_index().equals("A")){
				mrd = new String(C.process(report_url+"contractBytypeT03_A.mrd",type));
			}else if(statisticsForm.getStart_index() != null && statisticsForm.getStart_index().equals("B")){
				mrd = new String(C.process(report_url+"contractBytypeT03_B.mrd",type));
			}
		}

		if(accessLevel.equals("A") || accessLevel.equals("D")){	
			if(blngt_orgnz_gb.equals("B")){
				mrd_param = 
					"/rp ["+blngt_orgnz_gb+"]"+
					"["+blngt_orgnz+"]"+
					"["+srch_dt1+"]"+
...
<script language=javascript for=Rdviewer event="HlinkClicked(Params , URL)">
reportPop(Params , URL);
</script> 
...
<div id="wrap">
	...
	<form:form id="frm" name="frm" method="post" autocomplete="off">
		...
		<!-- Title -->
		<div class="title">
		<h3><spring:message code="las.page.title.report.StatisticsContractByType" /></h3>
		</div>
						...
						<colgroup>
							...
							<table class="search_form">
								...
								<tr>
		    	        		    ...
		    	        		    <th><spring:message code="las.page.title.report.ContractType" /></th>
		    	        		    <td>
								    <select name="cntrtType" id="cntrtType"  style='width:80'></select>
		    	        		    </td> 		    	        		    										
								</tr>	
								 <!-- 							
							    <tr>
								    <th>차트유형</th>
								    <td colspan="4">
								    <select name="start_index" id="start_index"  style='width:145'>
								    <option value='' selected>&nbsp;</option>	
	  							    <option value='A' <%=(statisticsForm.getStart_index() != null && statisticsForm.getStart_index().equals("A")?"selected":"") %>>A형</option>
	  							    <option value='B' <%=(statisticsForm.getStart_index() != null && statisticsForm.getStart_index().equals("B")?"selected":"") %>>B형</option>
		    	        		    ...
		    	        		    <th>권한테스트</th>
		    	        		    <td colspan="3">
		    	        		    <select name="end_index" id="end_index"  style='width:145'>
		    	        		    
								    <option value='' selected>&nbsp;</option>	
	  							    <option value='A' <%=(statisticsForm.getEnd_index() != null && statisticsForm.getEnd_index().equals("A")?"selected":"") %>>전사관리자</option>
	  							    <option value='B' <%=(statisticsForm.getEnd_index() != null && statisticsForm.getEnd_index().equals("B")?"selected":"") %>>조직관리자</option>
	  							    </select>
		    	        		    </td>
