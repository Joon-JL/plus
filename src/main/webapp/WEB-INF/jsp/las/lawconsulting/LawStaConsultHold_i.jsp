...
<script language="javascript">
    ...
    function send(){
        ...
        if(f.isGrpmgr.value == 'Y'){
            var confirmMessage = "<spring:message code='secfw.msg.ask.refer' />" ;
        }
        else{
            var confirmMessage = "<spring:message code='secfw.msg.ask.hold' />" ;
        }
...
<div id="wrap">
  ...
  <div id="container">
    ...
    <div class="title">
      ...
      <c:choose>
        <c:when test="${command.isGrpmgr == 'Y'}">
          <h3><spring:message code="las.page.field.lawconsulting.stdConRjtIns" /></h3>
        </c:when>
        <c:otherwise>
          <h3><spring:message code="las.page.field.lawconsulting.stdConHoldIns" /></h3>
        </c:otherwise>
      </c:choose>
    </div>
    <!-- content -->
	...
	<input type="hidden" id="isStdCont" name="isStdCont" value="Y"/>
      ...
      <div class="t_titBtn">
        <div class="btn_wrap">
          <span class="btn_all_w"><span class="reject"></span><a href="javascript:send()"><spring:message code="las.page.button.lawconsult.send" /></a></span>
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="table-style01">
        ...
        <tbody>
          ...
          <span id="curByteExpl_body_mine">0</span>/<spring:message code="clm.page.msg.common.max1000byte" /><br>
          <c:choose>
            <c:when test="${command.isGrpmgr == 'Y'}">
              <tr>
                <th><spring:message code="las.page.field.lawconsult.rejctcause" /></th>
                <td><textarea name="rejct_cause" id="rejct_cause" rows="16" class="text_area all" onkeyup="frmChkLenLang(this,1000,'curByteExpl_body_mine','<%=langCd%>')" alt="<spring:message code="las.page.field.lawconsulting.rejectRs"/>"></textarea></td>
              </tr>
            </c:when>
            <c:otherwise>
			  <tr>
                <th><spring:message code="las.page.field.lawconsult.holdcause" /></th>
                <td><textarea name="hold_cause" id="hold_cause" rows="16" class="text_area all" onkeyup="frmChkLenLang(this,1000,'curByteExpl_body_mine','<%=langCd%>')" alt="<spring:message code="las.page.field.lawconsulting.pstpnRs"/>"></textarea></td>
              </tr>
            </c:otherwise>
          </c:choose>
        </tbody>
