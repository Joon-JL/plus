...
function init(Msg){
	...
	if("<%=StringUtil.bvl(command.getUser_id(),"")%>" != ""){
		...
		$("#auth_apnt_yn").val('<%=StringUtil.bvl(command.getAuth_apnt_yn(),"")%>');
		
		var tmp = '<%=StringUtil.bvl(command.getAuth_apnt_yn(),"")%>';
...
function pageAction(div){
	...
	if(div == "search"){		
		...
		frm.method.value = "listSignDocUserMng";
		frm.submit();
		frm.target = "_self";
		...
		frm.target		 = "_self";
		frm.action       = "<c:url value='/secfw/userMng.do' />";
		frm.method.value = "SaveSignUserMng";
		frm.submit();
		frm.target = "_self";
	}
}
...
function setInfo(val){
	...
	if(tr_cnt ==1 && val == 0){
		...
		$("#auth_apnt_yn").val(frm.auth_apnt_yn_arr.value);
		
		if(frm.auth_apnt_yn_arr.value != ''){
			
			$("#auth_apnt_dept_nm").val(frm.auth_apnt_dept_nm_arr.value);
			$("#auth_apnt_dept").val(frm.auth_apnt_dept_arr.value);
			if(frm.auth_apnt_yn_arr.value == "Y"){
				$('#picker').attr("style", "cursor:hand;");
			}else{
				$('#picker').attr("style", "cursor:hand;display:none");
			}
		...
		$("#auth_apnt_yn").val(frm.auth_apnt_yn_arr[val].value);
		
		if(frm.auth_apnt_yn_arr[val].value != ''){
			
			$("#auth_apnt_dept_nm").val(frm.auth_apnt_dept_nm_arr[val].value);
			$("#auth_apnt_dept").val(frm.auth_apnt_dept_arr[val].value);
			if(frm.auth_apnt_yn_arr[val].value == "Y"){
				$('#picker').attr("style", "cursor:hand;");
			}else{
				$('#picker').attr("style", "cursor:hand;display:none");
			}
...
<div id="wrap">
	...
	<!-- title -->
	<div class="title">
		<h3><spring:message code="clm.page.field.signdoc.issuermng"/></h3>
	</div>
	<!-- //title -->
	...
	<div id="content">
		...
		<!-- 증명서류발급 화면코드 -->
		<input type="hidden" name="gubn_cd"  value="SC0102">	
					...
					<tr>
						<td>
							...
							<table class="search_form">
							   ...
							   <tr>
								   <th><spring:message code="clm.page.field.signdoc.issuer"/></th>
								   <td>
										...
										<input class="text w_50" type="text" name='srch_cntnt' id='srch_cntnt' style='width:90%' value="<%=StringUtil.bvl(command.getSrch_cntnt(), "")%>" maxlength='24' onKeyPress='if(event.keyCode==13) {pageAction("search");return false;}'/>
									</td>
									<th class='paddLeft10' align='right'><spring:message code="secfw.page.field.user.authYesno"/></th>											
									<td>
										<select name='srch_auth_apnt_yn' id='srch_auth_apnt_yn'>
											<option value=''></option>
											<option value='Y' <%=("Y".equals(command.getSrch_auth_apnt_yn()))?" selected ":"" %>>Y</option>
											<option value='N' <%=("N".equals(command.getSrch_auth_apnt_yn()))?" selected ":"" %>>N</option>
										</select>	
									</td>
								</tr>
							</table>
		...
		<div class='tableone'>
			...
			<tr id="UserListTitle">
				...
				<th>SingleID</th>
				<th><spring:message code="secfw.page.field.user.authYesno"/></th>				
		...
		<table class="list_scr">
			...
			<tr>
				...
				<td  align="center" ><%=lom.get("auth_apnt_yn")%></td>			
				...
				<input type="hidden" name="auth_apnt_yn_arr" id="auth_apnt_yn_arr" value="<%=lom.get("auth_apnt_yn")%>">
     ...
     <table class="list_basic mt20">
			...
			<tr>
					...
					<th class='head'><spring:message code="secfw.page.field.user.authYesno"/></th>
	          	<td>
					<select name='auth_apnt_yn' id='auth_apnt_yn' onChange="setApntYn(this.value)">
						<option value=''></option>
			...
			<tr>
				<th class='head'><spring:message code="clm.page.field.sign.corp"/><!-- 관리사업장--></th>
	          	<td><input type="text" name='ara_nm' id='ara_nm' class="text w_50"  maxlength="33"  value=""></td>
			</tr>
			<tr>
				<th class='head'><spring:message code="clm.page.msg.common.note"/><!-- 비고--></th>
	          	<td><input type="text" name='comments' id='comments' class="text w_50"  ></td>
			</tr>
		</table>
