...
<%
	...
	//댓글여부(Y:댓글)
	String isReply = StringUtil.bvl((String)request.getAttribute("isReply"),"N");

	ArrayList getBBSDetail      = (ArrayList)request.getAttribute("detailBBSDetail");
	String noticeId = "";
...
%>
		    ...
		    frm.method.value = "insertBBSDetail";
			frm.submit();
		// 댓글등록
		} else if(flag == "reply"){
<%
...
%>
			...
			frm.target = "_self";
			frm.action = "<c:url value='/secfw/bbs.do' />";
		    frm.method.value = "insertBBSDetailReply";
			frm.submit();
...
<input type="hidden" name="up_notice_id"          value="<%=noticeId %>" />
		...
		<table class="detail_form">
			...
			<col />
	          ...
	          <tr>
	            ...
	            <td>
	            	<span><input name="notice_title" id="notice_title" type="text" class="text" size="50" /></span>
<%
	if("Y".equals(urgencyYn) && !"Y".equals(isReply)) {
%>
...
<%
	if("Y".equals(holdTermYn) && !"Y".equals(isReply)) {
%>
	          <tr>
	            <th><spring:message code="secfw.page.field.bbs.effectiveDate"/><span class="astro">*</span></th>
	            <td>
	              	<input type="text" name="notice_start_ymd" id="notice_start_ymd"  class="input_calendar" value="<%=DateUtil.formatDate(DateUtil.today(),"-") %>" /><img src="<c:url value='/images/secfw/ico/ico_input_calendar.gif' />" onClick="popFrame.fPopCalendar(notice_start_ymd,notice_start_ymd,popCal)" class="cp">
		            &nbsp;~&nbsp;
	                <input type="text" name="notice_end_ymd" id="notice_end_ymd"  class="input_calendar" value="<%=DateUtil.formatDate(DateUtil.addDays(DateUtil.today(),30),"-") %>" /><img src="<c:url value='/images/secfw/ico/ico_input_calendar.gif' />" onClick="popFrame.fPopCalendar(notice_end_ymd,notice_end_ymd,popCal)" class="cp">
	            </td>
	          </tr>
<%
	}
%>
	          <tr>
	            <td colspan="2" class="pd" id="">
	            	<textarea name="notice_cntnt" id="notice_cntnt" cols="45" rows="20" class="text all"></textarea>
	            </td>
	          </tr>
<%
	if("Y".equals(fileYn)) {
...
<%
	...
	if("Y".equals(isReply)) {
%>
	      	<a href="javascript:pageAction('reply')"  class="bt_all_b"><span><img src="<c:url value='/images/secfw/ico/ico_save.gif' />"><spring:message code="secfw.page.field.bbs.save"/></span></a> 
<%
	} else {
%>
...
%>
    	  	<a href="javascript:pageAction('reset')" class="bt_all_w"><span><img src="<c:url value='/images/secfw/ico/ico_reset.gif' />"><spring:message code="secfw.page.field.bbs.init"/></span></a> 
