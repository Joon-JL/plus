...
import com.sec.las.lawconsulting.dto.SpeakConsultForm;
import com.sec.las.lawconsulting.dto.SpeakConsultVO;
import com.sec.las.lawconsulting.service.SpeakConsultService;
...
* Date : 2011.08.17
*/
public class SpeakConsultController extends CommonController {
	 ...
	 * 구두계약 서비스
	 */
	private SpeakConsultService speakConsultService;

	/**
	 * 구두계약 서비스 세팅
	 * 
	 * @param speakConsultService
	 */
	public void setSpeakConsultService(
			SpeakConsultService speakConsultService) {
		this.speakConsultService = speakConsultService;
	}
	 ...
	 * @throws Exception
	 */
	public ModelAndView listSpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		SpeakConsultForm form = new SpeakConsultForm();
		SpeakConsultVO vo = new SpeakConsultVO();
		// 데이터 바인딩
		bind(request, form);
		bind(request, vo);
		...
		return listSpeakConsult(request, vo, form);
	}
	 ...
	 */
	public ModelAndView listSpeakConsult(HttpServletRequest request,
			SpeakConsultVO vo, SpeakConsultForm form) throws Exception {
			String forwardURL = "/WEB-INF/jsp/las/lawconsulting/SpeakConsult_l.jsp";
			PageUtil pageUtil = new PageUtil();
			ModelAndView mav = new ModelAndView();
			String returnMessage = "";
		try {
			// 국내/해외 구분 변수 세팅
			if (form.getIsForeign().equals("F"))
				vo.setDmstfrgn_gbn("F");
			else if (form.getIsForeign().equals("H"))
			...
			// 리스트 조회 서비스 호출
			List<?> resultList = speakConsultService.listSpeakConsult(vo);
			...
			//여기서 등록권한 세팅
			//form.setAuth_insert(ClmsBoardUtil.getClassMethodAuth(request, classMethodAuthService, this.getClass().getName(), "insertSpeakConsult")) ;
			if(ClmsBoardUtil.searchRole(request, "RA01") || ClmsBoardUtil.searchRole(request, "RA02")) 
				form.setAuth_insert(true);
			else
				form.setAuth_insert(false);
	 ...
	 * @throws Exception
	 */
	public ModelAndView forwardInsertSpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		try {
			String forwardURL = "/WEB-INF/jsp/las/lawconsulting/SpeakConsult_i.jsp";
	 ...
	 * @throws Exception
	 */               
	public ModelAndView insertSpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (Token.isValid(request)) {
				...
				result = speakConsultService.insertSpeakConsult(vo);
			} else {
				// 중복 등록시 result값 세팅
				result = 100;
			}
			...
			if (result > 0) {
				...
				mav = listSpeakConsult(request, vo, form);
			...
			mav.setViewName("/WEB-INF/jsp/las/lawconsulting/SpeakConsult_i.jsp");
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	public ModelAndView forwardDetailSpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		SpeakConsultForm form = new SpeakConsultForm();
		SpeakConsultVO vo = new SpeakConsultVO();
		...
		return forwardDetailSpeakConsult(request, form, vo,
	 ...
	 */
	public ModelAndView forwardDetailSpeakConsult(HttpServletRequest request,
			SpeakConsultForm form, SpeakConsultVO vo, Locale locale)
			throws Exception {
		ModelAndView mav = new ModelAndView();
		String forwardURL = "/WEB-INF/jsp/las/lawconsulting/SpeakConsult_d.jsp";
		String returnMessage = "";
		...
		try {
			...
			lom = speakConsultService.detailSpeakConsult(vo); 
			...
			// 수정권한 세팅
			form.setAuth_modify(ClmsBoardUtil.getDeleteModifyAuth(request, (String)lom.get("respman_id"))) ;
			
			// 삭제권한 세팅
			form.setAuth_delete(ClmsBoardUtil.getDeleteModifyAuth(request, (String)lom.get("respman_id"))) ;
	 ...
	 * @throws Exception
	 */
	public ModelAndView forwardModifySpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		SpeakConsultForm form = new SpeakConsultForm();
		SpeakConsultVO vo = new SpeakConsultVO();
		String forwardURL = "/WEB-INF/jsp/las/lawconsulting/SpeakConsult_i.jsp" ;
		...
		try {
			...
			//조회결과가 있으면 받아온 데이터 중 수정이 가능한 데이터를 FORM에 세팅한다
				...
				form.setReqman_id((String)lom.get("reqman_id"));
				...
				form.setReqman_mail((String)lom.get("reqman_mail"));
				form.setReqman_dept((String)lom.get("reqman_dept"));
	 ...
	 * @throws Exception
	 */
	public ModelAndView modifySpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
			...
			try {	
				...
				if(Token.isValid(request)){
					...
					result = speakConsultService.modifySpeakConsult(vo) ;
				} else {
				...
				if(result>0){
					...
					mav = forwardDetailSpeakConsult(request, form, vo, new RequestContext(request).getLocale()) ;
	 ...
	 * @throws Exception
	 */
	public ModelAndView deleteSpeakConsult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		try {
			...
			if(Token.isValid(request)){
				//중복 삭제가 아닐 시
				result = speakConsultService.deleteSpeakConsult(vo);
			} else {
				//중복 삭제 시
				result = 100;
			}
	...
	private void setInitFormVO(HttpServletRequest request,
			SpeakConsultForm form, SpeakConsultVO vo) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void sendMailToAdmin(HttpServletRequest request, SpeakConsultVO sVo) {
		try {
			...
			if(sVo.getReqman_mail() != null){
				if(sVo.getReqman_mail().length() > 0){
					...
					//[구두자문] 담당자로부터 답변이 도착하였습니다.
					hm.put("main_title", (String)messageSource.getMessage("las.page.field.speakconsult.sendMailToAdmin01", null, lc));
					//건 답변이 도착했으니 확인바랍니다.
					hm.put("param1", sVo.getTitle() + (String)messageSource.getMessage("las.page.field.speakconsult.sendMailToAdmin02", null, lc));
			...
			// 제목
			subject = (String)messageSource.getMessage("las.page.field.speakconsult.sendMailToAdmin01", null, lc);
...
//			esbMailService.sendMail(vo.getModule_id(), vo.getMis_id());
	 ...
	 * @throws Exception
	 */
	private void setTopRole(HttpServletRequest request, SpeakConsultForm form, SpeakConsultVO vo) throws Exception {
