...
import com.sds.secframework.auth.dto.ClassMethodAuthForm;
...
public class ClassMethodAuthController extends CommonController{
	...
	public ModelAndView listPage(HttpServletRequest request, HttpServletResponse response) throws Exception{
		ClassMethodAuthForm form = new ClassMethodAuthForm();
	 ...
	 * @throws Exception
	 */
	public ModelAndView listPage(HttpServletRequest request, ClassMethodAuthForm form, ClassMethodAuthVO vo) throws Exception{
		String forwardURL = "/WEB-INF/jsp/secfw/auth/ClassMethodAuthList.jsp";
		BoardPageUtil pageUtil = new BoardPageUtil() ;
		ModelAndView mav = new ModelAndView();
		List ClassMethodAuthList = null;
		...
		//서비스 수행
		ClassMethodAuthList = ClassMethodAuthService.listPage(vo);
		form.setClass_method_auth_list(ClassMethodAuthList);

		//페이징 처리
		pageUtil.setTotalRow(pageUtil.getTotalRow(ClassMethodAuthList, "TOTAL_CNT"));
		pageUtil.setGroup() ;
	...
	public ModelAndView forwardInsertForm(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		ClassMethodAuthForm form = new ClassMethodAuthForm() ;
		ClassMethodAuthVO vo = new ClassMethodAuthVO();
		String forwardURL = "/WEB-INF/jsp/secfw/auth/ClassMethodAuthInsert.jsp" ;
		List availableAuthList = null;
		...
		availableAuthList = ClassMethodAuthService.listNotRole(vo);
		form.setAvailable_auth_list(availableAuthList);
		form.setAssigned_auth_list(null);
	...
	public ModelAndView forwardModifyForm(HttpServletRequest request, ClassMethodAuthForm form, ClassMethodAuthVO vo) throws Exception{
		ModelAndView mav = new ModelAndView();
		Map map = null;
		String returnMessage = null;
		List availableAuthList = null;
		List assignedAuthList = null;
		String forwardURL = "/WEB-INF/jsp/secfw/auth/ClassMethodAuthInsert.jsp" ;
		...
		//설정권한리스트 초기화
		availableAuthList = ClassMethodAuthService.listNotRole(vo);
		assignedAuthList = ClassMethodAuthService.listRole(vo);
		form.setAvailable_auth_list(availableAuthList);
		form.setAssigned_auth_list(assignedAuthList);
	...
	public ModelAndView insert(HttpServletRequest request, HttpServletResponse response) throws Exception{
		ClassMethodAuthForm form = new ClassMethodAuthForm();
		ClassMethodAuthVO vo = new ClassMethodAuthVO();
		String[] assigned_auth_list = request.getParameterValues("sel_assigned_auth_list");
		String[] available_auth_list = request.getParameterValues("sel_available_auth_list");
		String forwardURL = "/WEB-INF/jsp/secfw/auth/ClassMethodAuthInsert.jsp";
		ModelAndView mav = new ModelAndView();
		String returnMessage = null ;
		int result = 0;
		...
		vo.setAssigned_auth_list(assigned_auth_list);
		vo.setAvailable_auth_list(available_auth_list);

		try{
			// 중복 수정 체크
			if(Token.isValid(request)){
				//중복 등록이 아닐 시 : 등록 서비스 호출
				result = ClassMethodAuthService.insert(vo);
			}
			//중복 등록 시
			else {
				result = 100;
			...
			if(result > 0){
				...
				if(result == -100){
					...
					setInitAuthList(form, available_auth_list, assigned_auth_list);
					mav.setViewName(forwardURL);
				}
				//삽입 실패 시
				else{
		...
		try{
			if(Token.isValid(request)){
				...
				//수정할 데이터가 존재하면
					...
					result = ClassMethodAuthService.modify(vo);
	...
	public ModelAndView delete(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		try{
			if(Token.isValid(request)){
				...
				ClmsDataUtil.debug("########################");
				ClmsDataUtil.debug("######################## size : " + map.size());
				...
				//삭제할 데이터 존재 시
					...
					result = ClassMethodAuthService.delete(vo);
			...
			if(Token.isValid(request)){
				...
				for(int i = 0 ; i < checkbox.length ; i++){
					...
					//삭제할 데이터 존재 시
						...
						result = ClassMethodAuthService.deleteMulti(vo);
	 ...
	 * @throws Exception
	 */
	private void setInitFormVO(HttpServletRequest request, ClassMethodAuthForm form, ClassMethodAuthVO vo) throws Exception {
	 ...
	 * 권한 설정 list 조작
	 * @param form
	 * @param available_auth_list
	 * @param assigned_auth_list
	 */
	private void setInitAuthList(ClassMethodAuthForm form, String[] available_auth_list, String[] assigned_auth_list){
		List availableAuthList = new ArrayList() ;
		List assignedAuthList = new ArrayList();
		Map map = null ;

		available_auth_list = available_auth_list==null ? new String[0] : available_auth_list ;
		assigned_auth_list = assigned_auth_list==null ? new String[0] : assigned_auth_list ;

		for(int i=0; i<available_auth_list.length; i++) {
			map = new HashMap() ;
			map.put("role_cd", available_auth_list[i]) ;
			availableAuthList.add(map) ;
		}

		for(int i=0; i<assigned_auth_list.length; i++) {
			map = new HashMap() ;
			map.put("role_cd", assigned_auth_list[i]) ;
			assignedAuthList.add(map);
		}
		form.setAvailable_auth_list(availableAuthList) ;
		form.setAssigned_auth_list(assignedAuthList) ;
	}
