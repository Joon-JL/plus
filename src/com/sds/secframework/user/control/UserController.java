								...
								if (!"".equals(forwardURL)) { // forwarding URL이 존재
									mav.setViewName(forwardURL);
										...
										if("Y".equals(accessYn)){
											...
											boolean otp = userService.getUserOTP(sysCd, userId);
											
											if (otp)
											{
												String otp_access = String.format("%04d", new Random().nextInt(10000));
												
												Map userInfo_Map = (Map)userInfo.get(0);
												String email_destination = "";
												if (userInfo_Map.get("email") != null) {
													email_destination = userInfo_Map.get("email").toString();
												}
												
												//getLogger().debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
												//getLogger().debug("{otp: \"" + otp_access + "\", email: \"" + email_destination + "\" }");
												//getLogger().debug("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
												System.out.println("");
												System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
												System.out.println("{otp: \"" + otp_access + "\", email: \"" + email_destination + "\" }");
												System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
												System.out.println("");
												
												Map params = new HashMap();
												params.put("destination", email_destination);
												params.put("subject", "[SELMS+] OTP");
												params.put("body", "Use the following OTP for SELMS+ access: " + otp_access);
												mailSendService.sendSimpleEmail(params);
												
												session.setAttribute("otp_access", otp_access);
												mav.setViewName("/las/intro_OTP.do"); //법무 시스템
											}
											else
											{
...
//				mav.setViewName("/las/introNotice.do"); //법무 시스템
	...
	private boolean processTrayInfo(HttpServletRequest request, HttpSession session) throws Exception {
		...
		// mySingle key 위치 설정
		byte[] baPublicKey = Utils.getPublicKey(propertyService.getProperty("secfw.url.mySingleKey") + strKeyFolder + "/mySingle_key");
